...
def _copy_frontend_cross_root_sources(root: Path, work_dir: Path) -> None:
    """frontend 源码存在 ../../… 越根引用（shared/、comfyui-ext/），闭包必须带上。"""
    for name in ("shared", "comfyui-ext"):
        source = root / name
        if source.is_dir() and not (work_dir / name).exists():
            shutil.copytree(
...
def _assert_frontend_closure(frontend_work: Path) -> None:
    """扫描 frontend/src 相对 import，校验逃出闭包根的目标在闭包内可解析。

    frontend 内部引用（含 ../ 单层）由拷贝的 frontend 自身保证，不校验；
    TS import 无后缀，按 tsc 规则依次尝试补扩展名与 index。"""
    pattern = re.compile(
        r"""(?:from|import)\s*\(?\s*['"]((?:\.\./)+[^'"]+)['"]"""
    )
    root = frontend_work.resolve()
    extensions = (".ts", ".tsx", ".js", ".jsx", ".json")

    def resolvable(target: Path) -> bool:
        if target.exists():
            return True
        for ext in extensions:
            if Path(str(target) + ext).is_file():
                return True
        return any(
            (target / f"index{ext}").is_file() for ext in extensions
        )

    src = frontend_work / "src"
    missing: list[str] = []
    for path in sorted(src.rglob("*")):
        ...
        for raw in pattern.findall(content):
            ...
            if root in target.parents or target == root:
                continue  # frontend 内部引用，闭包自带
            if resolvable(target):
                continue
            missing.append(
                f"{path.relative_to(frontend_work).as_posix()} -> {raw}"
            )
    if missing:
        raise RuntimeError(
            "前端闭包缺少越根引用目标（tsc/vite 将无法解析）：\n"
            + "\n".join(missing)
        )
...
def build_frontend(root: Path, work_dir: Path, *, install_deps: bool) -> Path:
    ...
    _copy_frontend_cross_root_sources(root, work_dir)
    _assert_frontend_closure(frontend_work)
    npm = npm_executable()
