...
- ComfyUI 自动插画开启时，每个真实 Roleplay 回复在同一次主生成中先完成独立达标的可见 `<content>`，再输出隐藏高潮计划与当前 Profile 完整成稿；隐藏块不显示、不计正文篇幅，显式 `maxTokens` 追加 800–1000 token。隐藏 JSON 解析前复用正文正则，成稿校验失败从同轮 `scene_spec` 本地编译，自动链路不再补调第二个文本模型。主模型漏计划时仍以 `missing_plan_fallback` 发请求；误选低强度结尾钩子时废弃同源成稿并按纠正高潮重建。从状态 `[在场]` 恢复角色及刷新恢复合同不变。
