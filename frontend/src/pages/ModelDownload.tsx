...
import { effectiveGlobalProxyUrl } from "../lib/modelProxy";
import {
...
function LinkDownloadTab({ settings }: { settings: Settings }) {
  ...
  const onFetchInfo = async () => {
    ...
    setLoadingInfo(true);
    try {
      const r = await fetchModelInfo(url.trim(), settings.hfToken, settings.civitaiToken, effectiveGlobalProxyUrl(settings));
  ...
  const onDownload = async () => {
    ...
    try {
      const r = await downloadModel({ url: dlUrl, modelType: type, modelsDir: dir, hfToken: settings.hfToken, civitaiToken: settings.civitaiToken, name: info?.name || "", proxy: effectiveGlobalProxyUrl(settings) });
