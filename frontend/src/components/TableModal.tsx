...
import { X, Upload, Plus, Trash2, RefreshCw, ListChecks, TableProperties, Settings } from "lucide-react";
import {
  listTables, importTableTemplate, addTableRow, updateTableRow, deleteTableRow,
  createTable, dropTable, setTableMeta, manualFillTables, CHRONICLE_TABLE_UID,
...
import {
  ...
  previewNormalizeChronicle, applyNormalizeChronicle,
  type ChronicleEntry,
} from "../api/narrative";
...
function TablePane({
  ...
  if (sel === CHRONICLE_TABLE) {
    return <ChroniclePane chron={chron} outputDir={outputDir} repoId={repoId}
      chat={chat} busy={busy} setBusy={setBusy} setErr={setErr} reload={reloadChron}
...
function ChroniclePane({ chron, outputDir, repoId, chat, busy, setBusy, setErr, reload, openManualFill }: {
  chron: ChronicleEntry[];
  outputDir: string;
  repoId: string;
  chat: ChatModelInput;
  ...
  // 区间整理（2026-09-13 用户定案）：先出计划再确认。同一回合被多条纪要覆盖时保留最新一条、
  // 封口其余；相邻批次共享边界层则顺延保宽（33-35 / 35-37 → 33-35 / 36-38）。
  const [note, setNote] = useState("");
  const range = (s: number, e: number) => (s === e ? `第${s}层` : `第${s}–${e}层`);
  // 缺口补跑（2026-09-13 用户要求「针对空缺建立索引」）：整理只做让位/顺延，缺口层没有
  // 任何纪要覆盖，必须单独补跑。走显式区间模式（ranges），只处理缺口这几层、不推游标。
  const fillGaps = async (gaps: Array<[number, number]>, prefix: string) => {
    const list = gaps.map(([s, e]) => range(s, e)).join("、");
    if (!window.confirm(`${prefix}缺口 ${list} 没有任何纪要覆盖。\n\n是否立即补跑这些层的纪要？`)) return;
    setBusy(true); setErr("");
    try {
      // cardName 传空：区间模式下不读/不写抽取游标，只需要所选表。
      const fill = await manualFillTables(
        outputDir, repoId, "", [CHRONICLE_TABLE_UID], 1, 3, null, chat, gaps);
      const failed = fill.failed_batches ?? [];
      setNote(`${prefix}缺口补跑：处理 ${fill.processed ?? 0} 层、新增纪要 ${fill.chronicles ?? 0} 条`
        + (failed.length ? `；⚠ ${failed.length} 批失败已跳过（${failed.join("；")}）` : "。"));
      reload();
    } catch (e) { setErr(String((e as Error).message)); }
    finally { setBusy(false); }
  };
  const onNormalize = async () => {
    ...
    try {
      const plan = await previewNormalizeChronicle(outputDir, repoId);
      if (plan.clean) {
        if (!plan.gaps.length) { setNote("索引无重复、无边界重叠，无需整理。"); return; }
        setNote(`索引无重复、无边界重叠；缺口 ${plan.gaps.map(([s, e]) => range(s, e)).join("、")}。`);
        await fillGaps(plan.gaps, "索引无重复、无边界重叠；");
        return;
      }
      const lines: string[] = [];
      if (plan.duplicates.length) {
        const dropped = plan.duplicates.reduce((n, g) => n + g.drop.length, 0);
        lines.push(`重复区间 ${plan.duplicates.map((g) => range(g.turn_start, g.turn_end)).join("、")}`
          + `：各保留最新一条，封口 ${dropped} 条旧条`);
      }
      if (plan.rebases.length) {
        lines.push(`边界重叠 ${plan.rebases.length} 条顺延保宽（`
          + plan.rebases.map((r) => `${r.old[0]}–${r.old[1]}→${r.new[0]}–${r.new[1]}`).join("、") + "）");
      }
      const gapNote = plan.gaps.length
        ? `\n缺口 ${plan.gaps.map(([s, e]) => range(s, e)).join("、")}（整理后另行补跑）` : "";
      if (!window.confirm(`${lines.join("\n")}${gapNote}\n\n是否覆盖？`)) return;
      const res = await applyNormalizeChronicle(
        outputDir, repoId, plan.duplicates.flatMap((g) => g.drop), plan.rebases);
      const done = `已整理：封口 ${res.sealed} 条重复纪要，顺延 ${res.rebased} 条区间。`;
      setNote(done);
      reload();
      if (plan.gaps.length) await fillGaps(plan.gaps, `${done} `);
    } catch (e) { setErr(String((e as Error).message)); }
    finally { setBusy(false); }
  };
  return (
    <>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 6, marginBottom: 6 }}>
        <button className="btn" disabled={busy} onClick={onNormalize}
          title="检测同一回合被多条纪要覆盖的情况，先出计划再决定是否覆盖">
          <ListChecks size={13} style={{ verticalAlign: "-2px", marginRight: 4 }} />整理索引
        </button>
      ...
      {note && <p className="card-empty" style={{ marginBottom: 6 }}>{note}</p>}
      <div className="table-scroll">
        <div className="card-grid">
          {chron.map((e) => (
              ...
              <span>
                ...
                <small>
                  ...
                  {item.entries !== undefined
                    ? `已 ${item.entries} 条 · 未覆盖 ${item.unrecorded} 层`
                    : `未记录 ${item.unrecorded} 层`}
                  {` · 上次 T${item.last_turn || 0}`}
                </small>
              </span>
            </label>
          ))}
