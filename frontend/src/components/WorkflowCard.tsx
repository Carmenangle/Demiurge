...
import {
  NODE_VERIFY_GAPS_MS, isNodeSizeFor, isSoloNodeGraph,
} from "../lib/workflowNodeGuard";
...
export function WorkflowCard({
  ...
  const handleDone = async (ops?: any[]) => {
    ...
    try {
      ...
      // 某个帧此刻没画着它的目标节点（被同源会话恢复污染/帧未就绪）→ request_node 取回 null，
      // 该节点会按模板原值提交、用户刚调过的参数静默丢失。这里必须说出来，不能装作成功。
      const missing = nodeIds.filter((_, i) => !values[i]);
      if (missing.length > 0) {
        onNotify(
          `有 ${missing.length} 个节点画布没读到内容（#${missing.join("、#")}），`
          + "它们会按模板原值提交。若刚在这些节点上调过参数，请点「更改」重开画布再确认。",
        );
      }
...
export function NodeCard({
  ...
  // 内容校验失败导致的重挂次数上限（跨 effect 累计）：用尽则交手动重载按钮，杜绝无限重挂。
  const verifyRemountRef = useRef(0);
  const MAX_VERIFY_REMOUNT = 2;
  ...
  useEffect(() => {
    ...
    let verifyStep = 0;   // 已完成的复检次数（对应 NODE_VERIFY_GAPS_MS 下标）
    ...
    // 载入后持续复检：ComfyUI 的同源会话恢复可能迟到（兄弟 iframe 把裁剪后的单节点图
    // 写进 localStorage/IndexedDB，本帧恢复后画布变成别人的节点），只校验一次会被
    // 「先对后错」漏掉 —— 表现为「节点 3 · #20 的框里是节点 2 · #18，比例还是 #20 的」。
    const scheduleNextVerify = () => {
      verifyStep += 1;
      if (verifyStep < NODE_VERIFY_GAPS_MS.length) scheduleVerify(NODE_VERIFY_GAPS_MS[verifyStep]);
    };
    const onMsg = (ev: MessageEvent) => {
      ...
      if (d.type === "ready") {
        ...
        setLoaded(true);
        verifyStep = 0;
        scheduleVerify(NODE_VERIFY_GAPS_MS[0]);
      } else if (d.type === "graph") {
        // ★ 必须校验节点 id，不能只数节点个数：被兄弟帧污染的单节点画布同样是 1 个节点。
        if (isSoloNodeGraph(d.payload, nodeId)) {
          scheduleNextVerify();
        } else if (retryRef.current < MAX_RETRY) {
          retryRef.current += 1;
          sendLoad();
          scheduleVerify(1500);
        } else if (verifyRemountRef.current < MAX_VERIFY_REMOUNT) {
          verifyRemountRef.current += 1;
          setRemount((r) => r + 1);
        } else {
          setFailed(true);   // 重挂也救不回来 → 交给用户手动重载
        }
      } else if (d.type === "node_size") {
        // 尺寸也得是本节点的：污染前那次 node_size 已经设对比例，别被别人的尺寸改坏。
        if (!isNodeSizeFor(d.payload, nodeId)) return;
    ...
    <div style={{ marginBottom: 10 }}>
      <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 4, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {failed && (
          <button
            className="icon-btn"
            title="ComfyUI 页面加载超时或画布内容校验失败，点此重新载入"
            onClick={() => {
              setFailed(false); setLoadTries(0); setComfyReady(false);
              verifyRemountRef.current = 0;
              setRemount((r) => r + 1);
            }}
