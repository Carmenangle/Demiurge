...
describe("上下文条数改名迁移", () => {
  it("旧存档 historyPerRole 键迁移到 contextCount", () => {
    const settings = importSettings(JSON.stringify({ historyPerRole: 9 }));
    expect(settings.contextCount).toBe(9);
  });

  it("新键 contextCount 优先", () => {
    const settings = importSettings(JSON.stringify({ contextCount: 4, historyPerRole: 9 }));
    expect(settings.contextCount).toBe(4);
  });

  it("缺失回退默认 6", () => {
    const settings = importSettings("{}");
    expect(settings.contextCount).toBe(6);
  });
});
