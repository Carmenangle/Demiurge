...
import {
  appendAudioSlot, appendImageSlot, appendTransitionSlot, appendUnknownMessages, applyInterruptedStop, bindMediaSlotPrompt, dropMediaSlot, markMediaSlotFailed, pickFreshAssistantMedia, pruneUnsubmittedMediaSlots, reduceChatStreamEvent, resetMediaSlotForRetry, resolveMediaSlot,
...
describe("reduceChatStreamEvent", () => {
  it("trace 与 delta 分别落「执行过程」面板与正文（trace 不污染最终正文）", () => {
    ...
    expect(messages[0].agentTrace).toEqual([{ text: "主管选择 image" }]);
  ...
  it("trace 过程行在 replace 后保留，供完成后回看（2026-09-03 可视化改进）", () => {
    ...
    expect(messages[0].agentTrace).toEqual([
      { text: "🤔 模型正在思考…" },
      { text: "🧩 回填 14 个套装提示词" },
    ]);
  ...
  it("artifacts 挂到当前 assistant 消息，replace 后仍叠加且快照可恢复（2026-09-08）", () => {
    let messages = reduceChatStreamEvent(base(), "bot", { type: "replace", text: "合集卡已完成" });
    ...
    // 再叠加一批：不覆盖而是累积（多轮落盘的产物都展示）
    messages = reduceChatStreamEvent(messages, "bot", {
      type: "artifacts",
      items: [{ kind: "worldbook", name: "世界书 · 续写", path: "D:/w/玫瑰与繁花/worldbook.json", size: 10 }],
    });

    expect(messages[0].text).toBe("合集卡已完成");
    expect(messages[0].artifacts).toHaveLength(3);
    expect(messages[0].artifacts![0].kind).toBe("card");
    // 只影响 bot 消息：user 消息不带 artifacts
...
describe("pickFreshAssistantMedia（2026-09-09 画布/资产桥：逐张上屏时挑出新产物触发刷新）", () => {
  const media = (id: string, over: Partial<ChatMessage> = {}): ChatMessage => ({
    id, role: "assistant", text: `套装 ${id}\n\nprompt`, image: `local://${id}.png`, ...over,
  });
  it("只返回本轮首次上屏的 assistant 图片/视频消息", () => {
    const seen = new Set<string>();
    const items: ChatMessage[] = [
      media("img-1"),
      { id: "plan-card", role: "assistant", text: "📋 已编译计划…" },
      media("img-2", { video: "local://2.mp4" }),
    ];
    const fresh = pickFreshAssistantMedia(items, seen);
    expect(fresh.map((m) => m.id)).toEqual(["img-1", "img-2"]);
    // 全部 id 都已登记 → 同批再轮询不重复派发
    expect(pickFreshAssistantMedia(items, seen)).toEqual([]);
  });
  it("纯文本 / user 角色 / 无媒体消息不触发刷新", () => {
    const seen = new Set<string>();
    const items: ChatMessage[] = [
      { id: "assistant-text", role: "assistant", text: "计划已编译" },
      { id: "user-img", role: "user", text: "", image: "local://u.png" },
      { id: "audio-only", role: "assistant", text: "", audio: "local://a.wav" },
    ];
    expect(pickFreshAssistantMedia(items, seen)).toEqual([]);
    expect(seen.size).toBe(3); // 即便不触发刷新，也登记避免未来误派
  });
  it("上一轮已上屏的图在后续快照中重复出现 → 不再重复报告（防刷新风暴）", () => {
    const seen = new Set<string>();
    pickFreshAssistantMedia([media("img-1"), media("img-2")], seen);
    // 后续快照 = 旧图 img-1 + 新图 img-3
    const fresh = pickFreshAssistantMedia([
      media("img-1"), media("img-3"),
    ], seen);
    expect(fresh.map((m) => m.id)).toEqual(["img-3"]);
  });
});

describe("applyInterruptedStop（2026-09-04 收口：跨轮续写识别信号）", () => {
  const msg = (over: Partial<ChatMessage>): ChatMessage => ({
    id: "bot1", role: "assistant", text: "", ...over,
  });
  it("有半成品正文 → 文本尾部补「（已打断）」", () => {
    const out = applyInterruptedStop([msg({ text: "她转身，衣袂翻飞。" })], "bot1");
    expect(out[0].text).toBe("她转身，衣袂翻飞。（已打断）");
  });
  it("空文本（无可续正文）→ 标「（已停止生成）」", () => {
    const out = applyInterruptedStop([msg({})], "bot1");
    expect(out[0].text).toBe("（已停止生成）");
  });
  it("已带「（已打断）」→ 不重复补（stopGenerating 与 reducer 竞态幂等）", () => {
    const out = applyInterruptedStop([msg({ text: "正文（已打断）" })], "bot1");
    expect(out[0].text).toBe("正文（已打断）");
  });
  it("已带「（已停止生成）」→ 不重复补", () => {
    const out = applyInterruptedStop([msg({ text: "（已停止生成）" })], "bot1");
    expect(out[0].text).toBe("（已停止生成）");
  });
  it("顶层媒体卡（工作流出图）→ 不改（不补正文标记）", () => {
    const media = msg({ image: "http://x/a.png" });
    const out = applyInterruptedStop([media], "bot1");
    expect(out[0]).toBe(media);
    expect(out[0].text).toBe("");
  });
  it("非目标消息不受影响", () => {
    const other = msg({ id: "other", text: "别的正文" });
    const out = applyInterruptedStop([other], "bot1");
    expect(out[0]).toBe(other);
  });
  it("interrupted 流事件经 reducer → 半成品补「（已打断）」", () => {
    const out = reduceChatStreamEvent([msg({ text: "半截正文" })], "bot1", { type: "interrupted" });
    expect(out[0].text).toBe("半截正文（已打断）");
  });
  it("interrupted 流事件空文本 → 标「（已停止生成）」", () => {
    const out = reduceChatStreamEvent([msg({})], "bot1", { type: "interrupted" });
    expect(out[0].text).toBe("（已停止生成）");
  });
});
