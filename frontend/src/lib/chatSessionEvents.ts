...
// 计划采集「逐张上屏」的画布/资产桥（2026-09-09 江瑶 14 套实锤）：后端每采一张就写一条
// plan_collect 图片消息 + 入 RAG。本会话轮询把消息逐条上屏时，需要知道「哪些是这一轮新到
// 的产物」→ 派发 laf-generation-saved 让画布/资产网格即时刷新（此前缺桥 → 产物不出画布、
// 不进资产库直到计划全部完成）。
// 返回本轮首次上屏的 assistant 图片/视频消息，并把 items 全部 id 记入 seenIds 去重：
// 同一条消息只触发一次刷新事件（重入/重复轮询不风暴）。
export function pickFreshAssistantMedia(
  items: ChatMessage[], seenIds: Set<string>,
): ChatMessage[] {
  const added = items.filter((message) => !seenIds.has(message.id));
  for (const message of added) seenIds.add(message.id);
  return added.filter((message) =>
    message.role === "assistant" && Boolean(message.image || message.video));
}
...
function appendAgentTrace(message: ChatMessage, line: string, detail?: string): ChatMessage {
  const lines = [...(message.agentTrace || []), { text: line, ...(detail ? { detail } : {}) }];
...
// 停止/中断落标记（2026-09-04 收口）：半成品文本补「（已打断）」后缀——它是下一轮后端
// _resume_interrupted_messages 触发「从断点续写」的识别信号。前端内存态消息是跨轮续写
// 的第一输入源（roleplay 下一轮 ctx.history 优先取前端上传的可见历史），停止后不加标记
// 则下一轮永远匹配不到续写、观感=从头再来。
// 标记文本须与后端一致（chat_snapshot.INTERRUPT_MARKER；前端无共享常量层，字面对齐）。
const INTERRUPT_MARKER = "（已打断）";
const STOP_MARKER = "（已停止生成）";

export function applyInterruptedStop(
  current: ChatMessage[], messageId: string,
): ChatMessage[] {
  return current.map((message) => {
    if (message.id !== messageId) return message;
    // 顶层媒体卡（工作流出图/视频/音频气泡）不补正文标记
    if (message.image || message.video || message.audio) return message;
    const text = message.text || "";
    if (text.endsWith(INTERRUPT_MARKER) || text.endsWith(STOP_MARKER)) return message;
    // 有半成品正文 → 补「（已打断）」供下轮断点续写；无可续内容 → 沿用「（已停止生成）」提示
    return text.trim()
      ? { ...message, text: text + INTERRUPT_MARKER }
      : { ...message, text: STOP_MARKER };
  });
}
  ...
  switch (event.type) {
    case "trace":
      ...
      return current.map((message) => message.id === botId
        ? appendAgentTrace(message, event.text, event.detail)
        : message);
    case "delta":
      return current.map((message) => message.id === botId
    ...
    case "artifacts":
      // 2026-09-08：fabric 交付物元数据（主卡/世界书）挂到当前 assistant 消息，
      // replace 之后到达也能叠加；消息随快照持久化，历史回放保留产物卡。
      return current.map((message) => message.id === botId
        ? { ...message, artifacts: [...(message.artifacts || []), ...event.items] }
        : message);
    case "error":
      return current.map((message) => message.id === botId
    ...
    case "rag_status":
      ...
      return current;
    case "interrupted":
      // 服务端确认中断（含未走停止按钮的断流路径）：半成品补「（已打断）」标记，
      // 供下一轮 roleplay 命中断点续写；空文本标「（已停止生成）」（幂等防双标）。
      return applyInterruptedStop(current, botId);
  }
}
