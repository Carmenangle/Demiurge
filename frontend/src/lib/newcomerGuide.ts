      ...
      },
{
  ...
  },
  {
    ...
    title: "固化流程：小说转合集卡",
    summary: "把小说原文交给智能编造，按规范转为合集卡条目",
    steps: [
      {
        title: "1. 拖入小说文本并发指令",
        text: "在「智能编造」对话里把小说原文粘进输入框，或直接拖入 .md / .txt（底部说明：可拖进输入框供 Agent 参考）。明确一句「按固化知识库的合集卡规范处理为条目，并写入本作品世界书」即可——Agent 会自动读规范 §1（卡目录三文件形态、list 容器、5 字段、视觉画像契约的「角色卡·<名>」/「【外貌】/【身材】/【穿着】」锚点）。\n\n尽量给出范围：「按第一章出条」「每人一个外貌条目」之类的限定，避免把整本一次性塞满。",
        image: "",
      },
      {
        title: "2. 审计划与批准执行",
        text: "Agent 会贴出「已阅读计划：intent + steps + 预算」——steps 一般是「读文档 → 解析人物/场景 → 写条目」三步，预算显示条目数与 token 估算。出现「离审批」点「批准执行」即可，CPU/LLM 配额与路径域校验照旧生效。",
        image: "",
      },
      {
        title: "3. 去作品世界书核查条目",
        text: "完成后到「资产管理 → 世界书」或「仓库绑定」里打开本作品的世界书，能看到 list 容器里的新条目：constant 条目每轮常驻、其余条目按 keys 命中 + 向量召回。视觉画像条目务必以「角色卡·<角色名>」开头、正文含【外貌】/【身材】/【穿着】三行首锚点——下游画布插画与生图链路依赖这个契约。\n\n想反过来验证知识库本身生效了，去 ⚙ 设置 → 智能体 → 固化知识库，能看到《固化02-小说转合集卡规范》（§1 字符格式 + §3.3 keys 质量）与《固化03-ST迁移规范》（§3.5 第二套流程）两份独立文档。",
        image: "",
      },
    ],
  },
  {
    ...
    title: "固化流程：角色卡世界书转化",
    summary: "把 ST/通用角色卡的内嵌世界书外拆为独立世界书",
    steps: [
      {
        title: "1. 设置路径（首步前提）",
        text: "「⚙ 设置 → 路径」里填好「角色卡文件夹」与「世界书文件夹」并保存——后面导入时，系统就会把卡内嵌世界书外拆到世界书文件夹（同名独立世界书）。没填的话卡内嵌书不会被外拆，会跟着卡走、不进入世界书条目管理。",
        image: "",
      },
      {
        title: "2. 导入角色卡（自动外拆）",
        text: "「资产管理 → 角色卡 → 导入角色卡」选一张 ST PNG/JSON（或通用 JSON 卡）。后端走 character.import_source（extract_worldbook=true）能力：把卡本体按 TavernCard V1/V2/V3 解析落源库，同时把内嵌世界书按规范 §1 外拆为兄弟 worldbook.json（运行时真源），并从卡里清掉 character_book。落盘形态是卡目录三文件：card.json + worldbook.json + regex.json。\n\n导入后卡变干净（has_worldbook=false），同名独立世界书出现在「世界书」列表；下次「仓库绑定」选这张卡时会自动加载这本书。",
        image: "",
      },
      {
        title: "3. 也可对话式精细转化",
        text: "想筛掉条目、改 keys、合并或拆分时，把卡（PNG/JSON）拖入智能编造对话，指令「按《固化03-ST迁移规范》§3.2 把这张卡的内嵌世界书独立出来」。Agent 会按规范给计划（含哪些条目留下、哪些合并、keys 是否重写），批后落盘。",
        image: "",
      },
    ],
  },
