...
// 固化流程 01/02/03/自定义：内容各自独立成节，但**左侧栏只占一项**
// （curingFlows，页内按流程分组讲解）——左栏不再被固化章节撑满。
// 2026-09-11 用户定案「自定义流程先去掉」曾暂缓展示；2026-09-14 补齐上线：
// 「04：自定义环节」讲没有现成链路时，Agent 在真实任务里提议并固化流程
// （配图 curing-process-6/7/8/9.png）。旧 id create-curing-process 的别名映射保留。
const CURING_FLOW_SECTIONS: GuideSection[] = [
  ...
  {
    id: "create-curing-process",
    title: "固化流程 04：自定义环节",
    summary: "清单里没有同类任务时，Agent 先问一句「要不要固化」——跑通后这次步骤就成了可重放配方",
    steps: [
      {
        title: "1. 提一个清单里没有的任务，用大白话讲清目标",
        text: "自定义流程**不是某种固定走法**：任何不在 01/02/03 里的同类任务都能走这条——不用你事先设计流程，把任务讲清楚、把材料给全，照常跑一次即可。材料有什么给什么（参考图 / 灵感卡 / 规范文档 / 小说 / ST 卡…），文档直接拖进「智能编造」对话，不必指明路径或工具；Agent 会先回读关键信息、再讲清它打算怎么做，确认后才动手。\n\n截图是一次真实示例（**示例不是定义**，换个任务同理）：给一张角色参考图，让 Agent 先出**灵感卡**把外貌钉死（月见八千代：银—粉渐变发、粉—青渐变瞳、双马尾 + 环形辫 + 内层抹胸、胸前海螺、肩头海兔），再把《服装搭配参考分析》这类规范文档交给它；换成「把当前作品的世界书条目、角色卡外貌与近期纪要整理合并成一个设定文档」也一样成立。\n\n要点：讲清目标 + 给全材料，剩下的交给 Agent——同一套机制既适用于设计讨论，也适用于整理、迁移、批量生成。",
        image: "onboarding/curing-process-6.png",
      },
      {
        title: "2. 边做边确认，把这次任务完整跑完",
        text: "中途 Agent 会把还没定的关键点先问出来，你只负责拍板——示例里问的是「参考图锚点用哪张」「拟了 8 套场景，要不要增删」；换成整理上下文的例子，问的就会是合并规则、章节顺序、产物文件名。\n\n需要动手的步骤（durable / expensive）照常弹审批，点「批准执行」即可。\n\n要点：**必须完整跑通一次**——中途取消或失败不会留下可保留的草稿配方，轨迹只在成功跑完后才固化成配方。",
        image: "onboarding/curing-process-7.png",
      },
      {
        title: "3. 提议固化流程：Agent 主动问一句",
        text: "当这次任务在已固化的流程清单里没有同类，Agent 会**先问一句**：要不要把这次任务固化成一条可复用流程？以后同类请求可以整条重放，省 token（示例里你刚说完「整理上方内容生成文档」，它就问了）。\n\n· 回复「**要**」——现在照常执行，跑完把这次的步骤固化成流程；\n· 回复「**直接跑**」——照常执行、不固化（默认，不回也走这条）。\n\n这就是自定义流程的入口：不用提前设计，先把事做完，再让 Agent 把跑通的步骤收成配方。\n\n要点：固化流程预设（配方）与固化知识库（规范）互相独立、互不替代——知识决定 Agent 怎么想，配方决定 Agent 怎么快跑。",
        image: "onboarding/curing-process-8.png",
      },
      {
        title: "4. 产出结果：产物落盘 + 配方保留",
        text: "执行完，对话下方直接给出产物：**文件卡**（预览 / 打开位置 / 下载），正文给出落盘路径与字节数，再附一份**内容概要**与「下一步」（示例：作品 `docs/` 下的 `超时空辉夜姬-月见八千代-服装生图提示词设计文档.md`，7559 字节，概要含需求整理 → 外貌锚点 → 提示词规范 → 场景草案 → 待确认清单）。\n\n如果上一步回了「要」，消息底部还会出现 **已保留固化流程《…》** 的确认条——这条配方可在「⚙ 设置 → 智能体 → 固化流程预设」里查看与删除；之后同类请求优先整条重放，durable / expensive 步骤照常走审批与配额。\n\n要点：概要里的「待确认清单」是这类任务的常规收尾——把没定的事列出来，比反复改稿省事。",
        image: "onboarding/curing-process-9.png",
      },
    ],
  },
];
...
const curingFlows: GuideSection = {
  id: "curing-flows",
  title: "固化流程",
  summary: "四条固化流程：批量生图 / 小说转合集卡 / ST 卡转合集卡 / 自定义环节",
