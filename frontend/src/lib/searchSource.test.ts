...
describe("联网搜索源选项", () => {
  it("默认是 auto（自动回落链），不是某个具体源", () => {
    ...
    // 2026-09-11 加入 moegirl（萌娘百科站内搜索，第二跳）：与后端 _AUTO_CHAIN
    // (bing-cn → moegirl → ddg) 对齐，专治 bing 对长尾新角色名的间歇性放松。
    expect(SEARCH_PROVIDER_OPTIONS.map((o) => o.value)).toEqual([
      "auto",
      "bing-cn",
      "moegirl",
      "ddg",
...
describe("normalizeSearchProvider", () => {
  it("合法值原样保留", () => {
    ...
    expect(normalizeSearchProvider("moegirl")).toBe("moegirl");
...
describe("searchProviderWireValue", () => {
  ...
  it("显式源名原样上线（后端按名精确取 Adapter，不回落）", () => {
    ...
    expect(searchProviderWireValue("moegirl")).toBe("moegirl");
