...
import { effectiveGlobalProxyUrl, globalProxyAddress, resolveModelProxy } from "../lib/modelProxy";
...
import { subscribePlanTaskActivities, approvePlanTask, cancelPlanTask, type PlanTaskActivity } from "../lib/planTaskActivity";
...
export function ChatView({
  ...
  const inputPlaceholder = {
    story: "推进剧情、描写行动或对话；剧情高潮点会自动生成插画内嵌。Enter 发送，图片用上方 + 添加或直接粘贴，文本文件（md/txt 等）可拖进输入框供 Agent 参考",
    generate: "说出你想要的：描述画面直接生图、贴图让它反推或改图、提问绘画；/w 可选专业工作流。Enter 发送，图片用上方 + 添加或直接粘贴，文本文件（md/txt 等）可拖进输入框供 Agent 参考",
  ...
  // 待审批计划卡（2026-09-06）：刷新页面后消息里的 [[plan:]] 任务卡丢失，这里订阅
  // 后端 /plans 轮询，把当前作品（repo.id）的 awaiting_approval 计划恢复为对话内任务卡。
  const [pendingPlan, setPendingPlan] = useState<PlanTaskActivity | null>(null);
  useEffect(() => {
    const repoId = repo?.id ?? "";
    if (!repoId) { setPendingPlan(null); return; }
    const unsub = subscribePlanTaskActivities((items) => {
      const mine = items.find((t) => t.status === "awaiting_approval" && t.repo_id === repoId);
      setPendingPlan(mine ?? null);
    });
    return unsub;
  }, [repo?.id]);
  ...
  const chat = {
    ...
    proxyUrl: resolveModelProxy(activeChat?.proxyMode, globalProxyAddress(settings), settings.proxyEnabled),
  };
      ...
      {pendingPlan && (
        <div className="pending-plan-card">
          <div className="pending-plan-info">
            <strong>计划待审批</strong>
            <span className="pending-plan-intent">{pendingPlan.intent}</span>
            <span className="pending-plan-progress">{pendingPlan.progress}</span>
          </div>
          <div className="pending-plan-actions">
            <button
              ...
              onClick={async () => {
                try {
                  await approvePlanTask(pendingPlan.id);
                  setPendingPlan(null);
                } catch (e) {
            ...
            <button
              ...
              onClick={async () => {
                try {
                  await cancelPlanTask(pendingPlan.id);
                  setPendingPlan(null);
                } catch (e) {
            ...
            {messages.map((m, mIdx) => (
              ...
              {m.role === "user" ? (
                  ...
                  onInsert={(text, card) => {
                    if (card) {
                      ...
                      if (selected.length > 0) {
                        ...
                        if (att.sourceUrl && imgProxy) {
                          att.imageUrl = proxyImageUrl(att.sourceUrl, imgProxy);
                        }
                        richRef.current?.insertInspirationCard(att);
                      }
                    } else {
                ...
                <AssistantMessage
                  ...
                  onFabricApprove={(ok, mode) => send({
                    text: mode === "continue"
                      ? "继续补写当前作品的合集卡：基于已有卡纲和已落盘条目，先读 _prep/charfacts 对应素材提取原文细节，把不达标条目扩充到角色 1800 / 机制 800 / 世界 700 / NSFW 400 字以上，禁止重写短版覆盖已有内容。"
                      : (ok ? "批准" : "取消"),
                    parts: [{ type: "text" as const, text: mode === "continue"
                      ? "继续补写当前作品的合集卡：基于已有卡纲和已落盘条目，先读 _prep/charfacts 对应素材提取原文细节，把不达标条目扩充到角色 1800 / 机制 800 / 世界 700 / NSFW 400 字以上，禁止重写短版覆盖已有内容。"
                      : (ok ? "批准" : "取消") }],
                    images: [],
                  })}
