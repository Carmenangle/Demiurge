...
import { NEWCOMER_GUIDE_SECTIONS, resolveGuideSection, type GuideSection, type GuideStep } from "../lib/newcomerGuide";
import {
...
function Step({
  step, sectionId, stepNumber, onActivate, level = 3,
}: {
  step: GuideStep; sectionId: string; stepNumber: number;
  onActivate: (link: GuideLink) => void; level?: 3 | 4;
}) {
  // 分组章节（固化流程）里分组标题占 h3，步骤降一级到 h4，层级不打架
  const Heading = (level === 4 ? "h4" : "h3") as "h3" | "h4";
  return (
    <section id={guideStepAnchorId(sectionId, stepNumber)} style={{ marginBottom: 24, scrollMarginTop: 12 }}>
      <Heading style={{ margin: "0 0 8px", fontSize: level === 4 ? 19 : 22, fontWeight: 600 }}>{step.title}</Heading>
...
function SectionCard({
  ...
  // 分组章节（固化流程）的步骤锚点用「扁平序号」，与 lib 的 guideSteps 展开顺序一致
  let stepNo = 0;
  return (
    <section
      id={`guide-${section.id}`}
    ...
    >
      ...
      {section.steps.length === 0 ? (
        ...
        section.groups.map((group, gi) => (
          <div key={group.title} style={{ marginTop: gi === 0 ? 0 : 26 }}>
            <h3 style={{ margin: "0 0 4px", fontSize: 22, fontWeight: 600 }}>{group.title}</h3>
            {group.summary && (
              <p style={{ margin: "0 0 10px", fontSize: 18, color: "var(--text-muted)" }}>{group.summary}</p>
            )}
            {group.steps.map((step) => {
              stepNo += 1;
              return (
                <Step key={stepNo} step={step} sectionId={section.id}
                  stepNumber={stepNo} onActivate={onActivate} level={4} />
              );
            })}
          </div>
        ))
...
export function NewcomerGuideView({
  ...
  // 旧章节 id（固化流程 01/02/03/创建）经别名映射到合并后的 curing-flows，旧 hash 不失效
  const active = resolveGuideSection(activeId) ?? NEWCOMER_GUIDE_SECTIONS[0];
