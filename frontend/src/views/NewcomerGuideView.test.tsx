...
const { fetchGuideDoc, GUIDE_FIXTURE } = vi.hoisted(() => ({
  fetchGuideDoc: vi.fn(),
  GUIDE_FIXTURE: [
  {
    id: "quick-start",
...
vi.mock("../lib/newcomerGuide", () => ({
  NEWCOMER_GUIDE_SECTIONS: GUIDE_FIXTURE,
  // 组件用 resolveGuideSection 解析章节（含旧 id 别名）；别名映射本身由
  // lib/newcomerGuide.test.ts 覆盖，这里只按 id 查 fixture。
  resolveGuideSection: (id?: string | null) => GUIDE_FIXTURE.find((s) => s.id === id),
}));
