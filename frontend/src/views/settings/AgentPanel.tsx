...
import { normalizeContextBudgets, DEFAULT_CONTEXT_COUNT, DEFAULT_SELFHEAL_ATTEMPTS, DEFAULT_PRESET_SAMPLING } from "../../stores/settings";
    ...
    <div className="settings-section">
      ...
      <div className="settings-subsection">
          ...
          <div className="field">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const safe = Number.isFinite(n) ? Math.min(50, Math.max(1, Math.round(n))) : DEFAULT_CONTEXT_COUNT;
                setDraft((current) => ({ ...current, contextCount: safe }));
              }}
            />
          </div>
          <div className="field">
        ...
        <p className="field-hint">
          ...
          上限填 0 表示「无上限」：历史全量入上下文不裁剪（剧情模式降低失败率用；实际量仍受「上下文条数」约束，不会失控）。
          「上下文条数」是用户与 AI 各自读取的最近消息条数（默认 6，即共 12 条），再在上限内裁剪；剧情模式恒为 1（只上一次剧情轮，/w 等工作流不打断锚点）。
        </p>
