...
import { normalizeContextBudgets, DEFAULT_COMPACT_TRIGGER_PCT, DEFAULT_SELFHEAL_ATTEMPTS, DEFAULT_PRESET_SAMPLING } from "../../stores/settings";
    ...
    <div className="settings-section">
      ...
      <div className="settings-subsection">
        ...
        <div className="theme-options" style={{ marginTop: 10 }}>
          <button
            type="button"
            className={kcfg.mode === "smart" ? "is-active" : ""}
            disabled={kcfgSaving}
          ...
          <button
            type="button"
            className={kcfg.mode === "always" ? "is-active" : ""}
            disabled={kcfgSaving}
      ...
      <div className="settings-subsection">
        ...
        <p className="field-hint" style={{ marginTop: 0 }}>
          2026-09-04 设置整合：温度/采样/输入输出上限的权威来源统一在 settings，预设（preset JSON）不再管理这些字段。
          默认值取自 GrayWill-0.46-demiurge.json 基线；保存写回 <code>user_state.json</code>，全局生效。
        </p>
        <h5 style={{ marginTop: 12, marginBottom: 6, fontSize: 13, color: "var(--text-muted)" }}>上下文预算</h5>
        <div className="field-row">
          ...
          <div className="field">
            <label>压缩触发百分比（0=关闭）</label>
            <input
              type="number"
              min={0}
              max={100}
              step={5}
              value={draft.compactTriggerPct}
              onChange={(e) => {
                const n = Number(e.target.value);
                const safe = Number.isFinite(n)
                  ? Math.min(100, Math.max(0, Math.round(n)))
                  : DEFAULT_COMPACT_TRIGGER_PCT;
                setDraft((current) => ({ ...current, compactTriggerPct: safe }));
              }}
            />
          </div>
          <div className="field">
        ...
        <p className="field-hint">
          ...
          上限填 0 表示「无上限」：历史全量入上下文不裁剪（仍受模型窗口钳制，不会失控）。
          有效历史预算 = min(上限, 模型输入窗口 − 输出预留)（窗口/预留取下方采样参数的 openaiMaxContext/openaiMaxTokens）。
          历史占用达到有效预算的「压缩触发百分比」时自动压缩：更早对话由 LLM 压成结构化摘要，近期原文保留（对齐市面 /compact；
          压缩失败自动退回机械裁剪，不丢历史）。2026-09-11 起「上下文条数」已废弃移除，上下文长度由 token 预算唯一决定。
        </p>

        <h5 style={{ marginTop: 14, marginBottom: 6, fontSize: 13, color: "var(--text-muted)" }}>采样参数</h5>
        <div className="field-row">
          <div className="field">
            <label>温度（0~2）</label>
            <input
              ...
              max={2}
              step={0.05}
              value={draft.presetSampling.temperature}
              onChange={(e) => {
                const n = Number(e.target.value);
                const next = Number.isFinite(n) ? n : DEFAULT_PRESET_SAMPLING.temperature;
                setDraft((current) => ({
                  ...current,
                  presetSampling: { ...current.presetSampling, temperature: Math.min(2, Math.max(0, next)) },
                }));
              }}
            />
          </div>
          <div className="field">
            ...
            <input
              ...
              max={1}
              step={0.01}
              value={draft.presetSampling.topP}
              onChange={(e) => {
                const n = Number(e.target.value);
                const next = Number.isFinite(n) ? n : DEFAULT_PRESET_SAMPLING.topP;
                setDraft((current) => ({
                  ...current,
                  presetSampling: { ...current.presetSampling, topP: Math.min(1, Math.max(0, next)) },
                }));
              }}
            />
          </div>
          <div className="field">
            ...
            <input
              ...
              min={1}
              step={1}
              value={draft.presetSampling.topK}
              onChange={(e) => {
                const n = Number(e.target.value);
                const safe = Number.isFinite(n) ? Math.max(1, Math.round(n)) : DEFAULT_PRESET_SAMPLING.topK;
                setDraft((current) => ({ ...current, presetSampling: { ...current.presetSampling, topK: safe } }));
              }}
            />
          </div>
          <div className="field">
            <label>频率惩罚 frequency_penalty（-2~2）</label>
            <input
              type="number"
              min={-2}
              max={2}
              step={0.1}
              value={draft.presetSampling.frequencyPenalty}
              onChange={(e) => {
                const n = Number(e.target.value);
                const next = Number.isFinite(n) ? n : DEFAULT_PRESET_SAMPLING.frequencyPenalty;
                setDraft((current) => ({
                  ...current,
                  presetSampling: { ...current.presetSampling, frequencyPenalty: Math.min(2, Math.max(-2, next)) },
                }));
              }}
            />
          </div>
          <div className="field">
            <label>重复惩罚 presence_penalty（-2~2）</label>
            <input
              type="number"
              min={-2}
              max={2}
              step={0.1}
              value={draft.presetSampling.presencePenalty}
              onChange={(e) => {
                const n = Number(e.target.value);
                const next = Number.isFinite(n) ? n : DEFAULT_PRESET_SAMPLING.presencePenalty;
                setDraft((current) => ({
                  ...current,
                  presetSampling: { ...current.presetSampling, presencePenalty: Math.min(2, Math.max(-2, next)) },
                }));
              }}
            />
          </div>
        ...
        <h5 style={{ marginTop: 14, marginBottom: 6, fontSize: 13, color: "var(--text-muted)" }}>输入与输出上限</h5>
        <div className="field-row">
          <div className="field">
            <label>上下文窗口 openai_max_context（tokens）</label>
            <input
              type="number"
              min={1000}
              step={1000}
              value={draft.presetSampling.openaiMaxContext}
              onChange={(e) => {
                const n = Number(e.target.value);
                const safe = Number.isFinite(n) ? Math.max(1000, Math.round(n)) : DEFAULT_PRESET_SAMPLING.openaiMaxContext;
                setDraft((current) => ({ ...current, presetSampling: { ...current.presetSampling, openaiMaxContext: safe } }));
              }}
            />
          </div>
          <div className="field">
            <label>单次输出 openai_max_tokens（tokens，受 cap_min 收敛到模型上限）</label>
            <input
              type="number"
              min={256}
              step={1000}
              value={draft.presetSampling.openaiMaxTokens}
              onChange={(e) => {
                const n = Number(e.target.value);
                const safe = Number.isFinite(n) ? Math.max(256, Math.round(n)) : DEFAULT_PRESET_SAMPLING.openaiMaxTokens;
                setDraft((current) => ({ ...current, presetSampling: { ...current.presetSampling, openaiMaxTokens: safe } }));
              }}
            />
          </div>
        </div>
        <p className="field-hint">
          采样参数透传给所有 chat 端点（roleplay / world / curator / answer）。openai_max_tokens
          仍受后端 <code>llm.cap_max_tokens</code> 收敛（min 语义）：请求上限 &gt; 模型上限按模型来，
          未登记模型不钳；GrayWill 600000 → deepseek-v4-flash 393216 由 cap_min 兜底。
        </p>
      </div>
