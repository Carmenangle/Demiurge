...
export function CanvasStageFlow({
  ...
  const embed = useMemo(() => resolvedEmbedModel(settings),
    [settings.embedModel, settings.systemProxyUrl, settings.proxyUrl, settings.proxyEnabled]);
  ...
  useEffect(() => {
    ...
    const proxy = effectiveGlobalProxyUrl(settings);
    ...
    inspirationCardsRef.current = next;
    persistNow({ cards: next });
  }, [messages, repoId, settings.proxyEnabled, settings.proxyUrl, settings.systemProxyUrl, findFreeSpot, persistNow, layoutLoaded]);
