...
// 后端 manual_table_fill.CHRONICLE_UID 的镜像：纪要表是虚拟表（真身是 narrative_store
// 的 chronicle.db），在填表请求里用这个 uid 指代它。
export const CHRONICLE_TABLE_UID = "__chronicle__";
...
export interface TableStatusItem {
  ...
  last_turn: number;
  selectable: boolean;
  // 仅纪要表携带（2026-09-13）：entries=实际条数，covered_to=最远覆盖回合。
  // unrecorded 对纪要表=未覆盖层数（区间并集口径），对通用表=游标口径。
  entries?: number;
  covered_to?: number;
}
...
export interface ManualFillResult {
  ...
  total_turns?: number;
  // 2026-09-13：每张表与已处理范围的重叠层数（确认弹窗明示覆盖后果）。
  overlap_turns?: Record<string, number>;
  processed?: number;
  calls?: number;
  applied?: number;
  chronicles?: number;
  // 2026-09-13：解析失败被跳过的批次描述（重试耗尽）。空数组 = 全部成功。
  failed_batches?: string[];
}

// 手动填表批次进度（run 期间前端轮询；running=false 表示当前没有在跑的任务）
export interface ManualFillProgress {
  running: boolean;
  batch_done: number;
  batch_total: number;
}

export function getManualFillProgress(outputDir: string, repoId: string) {
  const q = new URLSearchParams({ output_dir: outputDir, repo_id: repoId });
  return apiGet<ManualFillProgress>(`/tables/manual-fill/progress?${q.toString()}`);
}
...
export function manualFillTables(
  ...
  chat: ChatModelInput,
  // 2026-09-13：缺口补跑用显式区间（[[start,end], ...]）；缺省/空 = 旧行为。
