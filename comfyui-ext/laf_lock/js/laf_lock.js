...
// ===== LOCK 模式：禁止把裁剪后的单节点图写回共享存储（串节点事故的源头）=====
// 同源的所有 LOCK iframe（同一张卡的不同节点、同页其它卡、甚至别的浏览器标签）共用一份
// localStorage，而 ComfyUI 监听 graphChanged 自动保存草稿（见前端
// src/platform/workflow/persistence/base/storageKeys.ts）：
//   Comfy.Workflow.DraftIndex.v2:personal           —— 索引（localStorage）
//   Comfy.Workflow.Draft.v2:personal:<hash(path)>   —— 草稿内容（localStorage）
//   Comfy.Workflow.ActivePath:<clientId>            —— 指针（sessionStorage，按 clientId 分键）
// loadGraphData 建的临时工作流路径固定是 "Unsaved Workflow.json" → hash(path) 全帧相同
// → 所有节点帧写的是同一个键：A 帧把裁剪后只剩 #18 的图存进去，B 帧迟到恢复会话时读回来的
//   就是 #18（现象：「节点 3 · #20 的框里是节点 2 · #18，比例还是 #20 的」）。
// LOCK 画布是临时的（父页面每次都重发 load，不需要任何会话恢复），所以从源头掐断写入。
// 只拦 localStorage 的草稿键；sessionStorage 的指针按 clientId 分键，互不影响，不拦。
const DRAFT_KEY_RE = /^(comfy\.workflow\.|workflow(:|$))/i;
function installPersistenceBlocker() {
  try {
    if (typeof Storage === "undefined" || Storage.prototype.__lafNoPersist) return;
    Storage.prototype.__lafNoPersist = true;
    const origSet = Storage.prototype.setItem;
    Storage.prototype.setItem = function (key, value) {
      try {
        if (this === window.localStorage && typeof key === "string" && DRAFT_KEY_RE.test(key)) return;
      } catch (e) {}
...
function startWatchdog(workflow, exposedIds) {
  ...
  watchdog = setInterval(async () => {
    ...
    // 守护窗口 ~30s（原 8s）：ComfyUI 的会话恢复来自同源 localStorage/IndexedDB，
    // 而兄弟 iframe（同卡其它节点、同页其它卡）也在往同一份存储写它们裁剪后的单节点图，
    // 恢复动作可能迟到 8s 之后才发生 —— 8s 窗口一过就被"先对后错"漏掉，
    // 表现为「节点 3 的框里是节点 2」。父页面另有 id 级复检兜底（workflowNodeGuard），
    // 这里延长窗口做第二道防线。
    if (tries >= 60) { clearInterval(watchdog); watchdog = null; }
  }, 500);
}
...
if (LOCK) {
  app.registerExtension({
    ...
    async setup() {
      ...
      installLoadGraphLock();
      installPersistenceBlocker();   // 先堵写，再清存量（否则清完又被自己写回去）
      await clearComfyStorage();
      hideChrome();
      installGlobalGuards();
      // 收父窗口消息
