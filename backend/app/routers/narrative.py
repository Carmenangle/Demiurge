...
def list_chronicle(output_dir: str, repo_id: str, k: int = 50) -> dict[str, object]:
    ...
    2026-09-13：卡号按**回合序**编号（T<层>-1 起），需在全库范围内定序——按分页子集
    编号会让每条都从 1 开始（每页都出现 T1-1），故先取全库序号表再回填本页。
    """
    items = narrative_store.recent(output_dir, repo_id, k=max(1, min(k, 200)))
    narrative_store.with_sequence(output_dir, repo_id, items)
...
def search_chronicle(req: SearchRequest) -> dict[str, object]:
    ...
    narrative_store.with_sequence(req.output_dir, req.repo_id, items)
    return {"items": [_dump(e) for e in items]}
...
# ── 区间整理（2026-09-13 用户定案）：重复归组 / 边界顺延，先出计划再确认 ──
...
def normalize_preview(req: NormalizeRequest) -> dict[str, object]:
    """区间整理**预览**（只读）：每回合覆盖计数、同区间重复组、边界重分配、缺口区间。

    用户定案「先出计划再确认」：本端点不落盘，前端展示后由用户决定是否覆盖。
    `total_turns` 缺省/≤0 时从会话快照取真实层数——否则缺口恒为空（2026-09-13 用户实锤
    「看不到针对空缺建立索引的功能」）。
    """
    if not (req.output_dir and req.repo_id):
        raise HTTPException(status_code=400, detail="缺少 output_dir 或 repo_id")
    total = max(0, req.total_turns)
    if total <= 0:
        from app.services import manual_table_fill

        total = manual_table_fill.session_turn_count(req.repo_id)
    return narrative_store.normalize_preview(req.output_dir, req.repo_id, total_turns=total)


class NormalizeApplyRequest(BaseModel):
    output_dir: str
    repo_id: str
    seal_rowids: list[int] = []
    rebases: list[dict[str, object]] = []


@router.post("/normalize/apply")
def normalize_apply(req: NormalizeApplyRequest) -> dict[str, object]:
    """落盘整理结果：封口让位的重复条目（可审计不物理删）+ 改写边界重叠区间。"""
    if not (req.output_dir and req.repo_id):
        raise HTTPException(status_code=400, detail="缺少 output_dir 或 repo_id")
    return narrative_store.normalize_apply(
        req.output_dir, req.repo_id, seal_rowids=req.seal_rowids, rebases=req.rebases)
...
def export_chronicle(output_dir: str, repo_id: str) -> dict[str, object]:
    ...
    含封口条目（可审计），故序号按**全量**集合定序，与界面可见列表口径一致。
    """
    items = narrative_store.all_entries(output_dir, repo_id)
    narrative_store.with_sequence(output_dir, repo_id, items, include_sealed=True)
