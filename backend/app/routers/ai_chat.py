...
class MessageEditRequest(BaseModel):
    thread_id: str = "home"
    message: dict = {}   # 完整消息对象（含 id），按 id 原子替换既有消息


@router.post("/chat/message/edit")
def chat_message_edit(req: MessageEditRequest) -> dict[str, object]:
    """按消息 id 原子落库单条编辑（用户编辑专用通道）。

    编辑走全量快照保存（save_if_newer）存在静默失败面（防抖/网络抖动被前端
    吞掉），且失败后下一轮生成会用旧快照把编辑打回（2026-09-13 用户实锤：
    删掉「（正文到此…）」后新轮次生成，旧文本重新出现）。低频高价值操作
    走单条原子更新；目标消息在快照中不存在时 updated=False（不制造幽灵消息）。
    """
    from app.services import chat_snapshot
    if not req.message.get("id"):
        raise HTTPException(status_code=400, detail="消息缺少 id")
    updated = chat_snapshot.edit_message(req.thread_id, req.message)
