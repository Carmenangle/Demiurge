...
CATEGORIES = (CATEGORY_COMFYUI, "worldbook", "character", "repo", "rag", "media", "asset",
              "novel", "narrative", "knowledge", "web", "task")
...
ENV_PARAM_SOURCES: dict[str, str] = {
    ...
    # 隔离材料消化（task.digest_materials，P3 子任务隔离 2026-09-12）：对话模型
    # 配置同样由运行环境决定，模型传值作废。机械计划路径不掌握 chat 配置 →
    # 默认空串注入（对齐 search_proxy 先例），能力侧回结构化错误。
    "chat_base": "chat_base",
    "chat_key": "chat_key",
...
ENV_INJECTED_OPS: dict[str, tuple[str, ...]] = {
    ...
    "task.digest_materials": ("chat_base", "chat_key", "chat_model"),
}
...
def env_param_value(key: str, *, output_dir: str, repo_id: str,
                    search_proxy: str = "", chat_base: str = "",
                    chat_key: str = "", chat_model: str = "") -> str:
    """环境注入参数的取值：作品根类取 output_dir，联网代理取请求上下文，
    对话模型配置取运行上下文（fabric 路径才有值），其余取 repo_id。

    search_proxy 默认空串：机械计划路径（plan_tasks/plan_compiler）不掌握联网代理，
    空值 = 直连，与既有行为一致（只有 fabric 自由循环会注入真实代理）。
    chat_* 同理：机械计划路径空串注入 → task.digest_materials 回结构化错误
    （LLM 消化本就只该在自由循环里发生）。
    """
...
def inject_env_params(params: dict, operation: str, *,
    ...
    for key in env_injected_params(operation):
        value = env_param_value(key, output_dir=output_dir, repo_id=repo_id,
                                search_proxy=search_proxy, chat_base=chat_base,
                                chat_key=chat_key, chat_model=chat_model)
        if value or force:
            params[key] = value
            written.append(key)
    return written
...
FABRIC_DOC_DELIVERY_OPS: frozenset[str] = frozenset({
    ...
    # 隔离材料消化（P3 子任务隔离，2026-09-12）：长材料阅读在独立干净上下文完成，
    # 只有摘要结论回主对话——治「整本 read_text 40k 字符/次淹没主上下文」的结构性解法
    # （此前只能靠提示词纪律 + 分卷读缓解）。readonly，审批零停。
    "task.digest_materials",
})
...
register(Capability(
    operation="task.digest_materials",
    category="task",
    description=(
        "隔离材料消化：把长文本材料（小说/长文档）的阅读放进独立干净上下文完成，"
        "只把结论返回本对话——材料原文不占用主对话历史。用途：按指令从材料提取"
        "角色素材、要点、结构、时间线等。instruction 写清楚要提取什么与输出格式；"
        "paths 传材料绝对路径（UTF-8 文本，≤8 个）。单文件读 60000 字符、总输入"
        "120000 字符封顶，超出截断并在 truncated 里列出。只读不落盘；对话模型"
        "未配置时返回结构化错误。"
    ),
    params_schema={
        "type": "object",
        "properties": {
            "instruction": {"type": "string",
                            "description": "消化指令：要提取什么、输出什么格式"},
            "paths": {"type": "array", "items": {"type": "string"},
                      "description": "材料绝对路径（UTF-8 文本）"},
            "max_digest_chars": {"type": "integer",
                                 "description": "摘要结论最大字符数（默认 4000）"},
        },
    ...
    side_effect_level=SIDE_EFFECT_READONLY,
    channel=CHANNEL_SYNC,
    handler="app.services.capability_handlers:digest_materials",
))
