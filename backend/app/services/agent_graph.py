...
def supervisor_node(state: AgentState) -> dict:
    ...
    if forced_route:
        route = forced_route if _route_available(forced_route, has_images, ctx) else chat_default
            ...
            (not has_images and plan_compiler.is_delegation_intent(intent_text))
            or plan_compiler.is_doc_delegation_intent(intent_text)
            or plan_compiler.is_design_discussion_intent(intent_text)
            or _task_session_continuation(ctx, intent_text)
            or plan_compiler.is_design_discussion_continuation(
                intent_text, _recent_user_texts(ctx))):
        # 路由界限·委派强命令层：高置信规模词+资产动作 / 显式计划语言 / 文档交付
        # 强命令 → 委派（零 LLM）。文档交付允许带图附件：参考图在此是素材
        # （看图反推→生成套装文档），不是图生图/反推的目标。
        # 2026-09-11 加设计讨论型：绑卡作品里「参考文档讨论各服装的生图提示词」
        # 此前掉进 chat_default=roleplay → 剧情楼层（实锤）；讨论的正道是智能编造
        # 自由循环。误判方向：纯剧情讨论不带设计话题词，不命中。
        # 2026-09-11 晚加讨论粘性：接续轮（「不用ComfyUI的提示词,我要适配
        # gpt/banana 的文字描述」）无任何讨论动词 → 三个判定全不命中 → 掉回
        # roleplay、灵感卡/规范/附件注入整段失效（隔一轮就失忆）。近 3 条用户
        # 消息里有设计讨论 + 本轮带设计话题词 + 非其他明确意图 → 仍走讨论循环。
        route = "plan"
        ctx["scene"] = scene_classify.infer_scene(text)
        run_trace.emit(ctx, "agent.completed", agent="supervisor", route="plan",
...
def _recent_inspiration_blocks(ctx: Any, limit: int = 2, chars: int = 700) -> str:
    """最近灵感卡 → 讨论上下文参考块（2026-09-11 实锤修复）。

    to_prompt_history 刻意跳过灵感卡（不污染剧情上下文），副作用是讨论型请求
    （「参考这个角色设计服装」）的 AI 看不到卡里的发色/瞳色/设定，反问用户要特征
    ——用户观感即「灵感图丢了/特征没理解」。此处只进自由循环历史（智能编造域），
    剧情通道不受影响。只搬标题+正文节选+已勾选图 URL。
    """
    ...
    cards: list[dict] = []
    try:
        for item in chat_snapshot.load(thread_id):
            if isinstance(item, dict) and isinstance(item.get("inspiration"), dict):
                cards.append(item["inspiration"])
    except Exception:
        return ""
    if not cards:
        return ""
    blocks: list[str] = []
    for insp in cards[-limit:]:
        title = str(insp.get("title") or "未命名")
        content = str(insp.get("content") or "")[:chars]
        selected = [u for u in (insp.get("selected") or []) if isinstance(u, str)]
        block = f"【灵感参考 · {title}】\n{content}"
        if selected:
            block += "\n已选参考图：" + "；".join(selected[:3])
        blocks.append(block)
    return ("【最近灵感卡（讨论时直接引用其特征，无需再向用户索要）】\n"
            + "\n\n".join(blocks))


def _task_session_continuation(ctx: Any, intent_text: str) -> bool:
    """上下文合同 P1：任务会话接续判定——先查登记（显式状态），词表只兜底。

    登记里该会话有活跃 design_discussion 任务 + 本轮带设计话题词 + 本轮不是
    其他明确意图（委派/文档交付）→ 仍走设计讨论循环。登记缺失/TTL 过期/
    任务类型不符一律 False（fail-open 退回词表粘性判定）。
    """
    from app.services import plan_compiler, task_session_store
    source = (intent_text or "").strip()
    if not source:
        return False
    if (plan_compiler.is_delegation_intent(source)
            or plan_compiler.is_doc_delegation_intent(source)):
        return False
    if not plan_compiler.mentions_design_topic(source):
        return False
    session = task_session_store.active(thread_id=str(ctx.get("thread_id") or ""))
    return bool(session and session.get("task_type") == "design_discussion")


def _recent_user_texts(ctx: Any, limit: int = 3) -> list:
    """快照里最近 N 条用户消息原文（讨论粘性判定用，不含本轮）。"""
    try:
        from app.services import chat_snapshot
        items = chat_snapshot.load(str(ctx.get("thread_id") or ""))
    except Exception:  # noqa: BLE001 - 快照缺失不阻断路由
        return []
    out = []
    for _m in items:
...
def _recent_attachment_lines(ctx: Any, limit: int = 5) -> str:
    """本会话此前上传的附件 → 真实路径引用块（讨论接续轮用）。

    附件挂在当轮 ctx、快照消息 parts（type=file）里有存档（2026-09-11 实锤）：
    文档在上一轮上传、接续轮 ctx 无 attachments → AI 回头找用户要
    「你文档中的服装设计内容」。从快照倒序捞 fileId 并解析真实路径。
    """
    try:
        from app.services import attachment_store, chat_snapshot
        items = chat_snapshot.load(str(ctx.get("thread_id") or ""))
    except Exception:  # noqa: BLE001
        return ""
    seen = {str(_a.get("file_id") or "") for _a in (ctx.get("attachments") or [])
            if isinstance(_a, dict)}
    lines: list = []
    for _m in reversed(items):
        if len(lines) >= limit:
            break
        if not (isinstance(_m, dict) and _m.get("role") == "user"):
            continue
        for _p in (_m.get("parts") or []):
            if not (isinstance(_p, dict) and _p.get("type") == "file"):
                continue
            _fid = str(_p.get("fileId") or "")
            _name = str(_p.get("name") or "")
            if not _fid or _fid in seen:
                continue
            seen.add(_fid)
            _path = ""
            try:
                _r = attachment_store.resolve(_fid)
                _path = str(_r) if _r is not None else ""
            except Exception:  # noqa: BLE001
                _path = ""
            lines.append(f"- {_name}（真实路径：{_path or '未落盘'}）")
    if not lines:
        return ""
    return ("【本会话此前上传的附件文件（接续轮仍可直接引用）】\n"
            + "\n".join(lines)
            + "\n直接使用上述真实路径读取/引用，禁止反复 list_dir 查找、"
              "禁止向用户索要本会话已上传过的文件。")


def _fabric_knowledge_history(ctx: Any, user_text: str,
                              task_type: str = "") -> str:
    ...
    退回原全量拼接，不误伤依赖上文的短指令。
    上下文合同 P1（2026-09-11）：原 include_inspirations 布尔升级为 task_type
    合同模板——"design_discussion" 注入 ②材料（灵感卡+历史附件真实路径）+
    ③规范（提示词规范全文），其余任务类型暂走基线装配。
    """
    ...
    # P2b（2026-09-12）：keep 档按 token 预算收口（同域长讨论不再无限膨胀），
    # 预算走主通道同款「min(设置, 模型窗口−预留)」；摘要器 fail-open 退机械聚合。
    _hist_budget = agent_context.resolve_history_budget(
        ctx.get("context_max_tokens") or 0, ctx.get("preset_sampling"))
    history_text = _cg.history_text_gated(ctx, intent_source, anchor_text=intent_source,
                                          embed_fn=_history_embed_fn(ctx),
                                          keep_token_budget=_hist_budget,
                                          keep_summarizer=_history_summarizer(ctx))
    ...
    if flow_hint:
        history_text = (history_text + "\n\n" + flow_hint).strip()
    # 上下文合同⑤产物指针（2026-09-12）：活跃任务会话里累积的已产出文件路径注入，
    # 接续轮「改一下/再补一份」直接改这些文件而非重生成。会话缺失/TTL 过期静默跳过。
    try:
        from app.services import task_session_store
        _sess = task_session_store.active(thread_id=str(ctx.get("thread_id") or ""))
        _prod = [str(_p) for _p in ((_sess or {}).get("product_paths") or []) if _p]
        if _prod:
            history_text = (history_text
                            + "\n\n【本任务已产出文件（更早轮次交付物，直接引用路径；"
                              "用户要求修改时改这些文件，勿重复生成）】\n"
                            + "\n".join(f"- {_p}" for _p in _prod[:10])).strip()
    except Exception:  # noqa: BLE001 - 产物指针缺失不影响装配
        pass
    if task_type == "design_discussion":
        insp_block = _recent_inspiration_blocks(ctx)
        if insp_block:
            history_text = (history_text + "\n\n" + insp_block).strip()
        # 设计讨论提示词规范注入（2026-09-11 用户四条质量反馈固化）：参考图锚点
        # 第一句 / 渐变特征恒定 / 三段式 tag+自然语言混排 / 长度下限。目前唯一
        # include_inspirations=True 调用方是设计讨论分支，故挂在此处随讨论生效；
        # 文档缺失/读取失败不阻断讨论。
        try:
            from app.services import agent_knowledge
            _spec = agent_knowledge.read_doc("设计讨论-角色服装提示词规范.md")
            if _spec.get("content"):
                history_text = (history_text
                                + "\n\n【设计讨论提示词规范（下方产出草案必须逐条遵守）】\n"
                                + str(_spec["content"])).strip()
        except Exception:  # noqa: BLE001 - 规范文档缺失不阻断讨论
            pass
        _hist_att = _recent_attachment_lines(ctx)
        if _hist_att:
            history_text = (history_text + "\n\n" + _hist_att).strip()
    attach_lines = []
...
def _fabric_finalize(outcome: Any, text: str, trace: list[str],
    ...
    if outcome.status == "done":
        ...
        try:
            ...
            pass
        # 上下文合同⑤产物指针回写（2026-09-12）：本轮实际写入的交付物路径进任务
        # 会话，接续轮注入「已产出文件」——用户说「改一下」时改这些文件而非重生成。
        # 已有活跃任务（讨论/委派）时保留其 task_type，只累积产物。
        try:
            from app.services import task_session_store
            _paths = [str(_a.get("path") or "") for _a in (artifacts or [])
                      if isinstance(_a, dict) and _a.get("path")]
            if _paths:
                _tid = str((ctx or {}).get("thread_id") or "")
                _prev = task_session_store.active(thread_id=_tid)
                task_session_store.save({
                    "task_type": (_prev or {}).get("task_type") or "fabric_delivery",
                    "intent": text,
                    "thread_id": _tid,
                    "output_dir": output_dir,
                    "product_paths": _paths[:10],
                })
        except Exception:  # noqa: BLE001 - 产物登记失败不影响交付
            pass
...
def _run_fabric_content_loop(ctx, state, *, intent_text, judge_text, text,
                             trace, output_dir, configured,
                             task_type: str = "") -> dict:
    """approval 模式内容生成/讨论型任务：fabric 自由循环断点审批。

    2026-09-11 自 doc 交付分支抽取：文档交付、设计讨论与「计划编译撞意图-步骤
    不一致」的智能降级共用同一循环（step_limit/error 自动续跑 ≤2 次，
    awaiting_approval 落断点）。task_type="design_discussion" 时按合同模板把
    最近灵感卡/历史附件/提示词规范注入讨论历史（上下文合同 P1）。
    """
    from app.services import capability_sandbox, fabric_checkpoint, fabric_loop
    ...
    history_text = _fabric_knowledge_history(ctx, text, task_type=task_type)
...
def plan_compiler_node(state: AgentState) -> dict:
    ...
    # 上下文合同 P2b：委派型任务同样登记（materials 并集/首链意图保留）——
    # 一线程一活跃任务，后一个任务自然覆盖前一个（设计讨论接续判定随之失效，
    # 词表粘性兜底）。路由不消费委派登记（委派意图本身判定可靠），仅作任务卡
    # ①目标/②材料/⑤产物的登记源；登记失败不阻断。
    if (plan_compiler.is_delegation_intent(intent_text)
            or plan_compiler.is_doc_delegation_intent(_fabric_judge)):
        try:
            from app.services import task_session_store
            task_session_store.save({
                ...
                "output_dir": output_dir,
                "materials": [
                    {"file_id": str(_a.get("file_id") or ""),
                     "name": str(_a.get("name") or "")}
                    for _a in (ctx.get("attachments") or [])
                    if isinstance(_a, dict) and _a.get("file_id")
                ],
            })
        except Exception:  # noqa: BLE001
            pass
    if plan_compiler.is_doc_delegation_intent(_fabric_judge):
        ...
        return _run_fabric_content_loop(ctx, state, intent_text=intent_text,
                                        judge_text=_fabric_judge, text=text,
                                        trace=trace, output_dir=output_dir,
                                        configured=configured)
    if (plan_compiler.is_design_discussion_intent(_fabric_judge)
            or _task_session_continuation(ctx, intent_text)):
        # 设计讨论型（2026-09-11）：讨论没有 submit/产出步骤，编机械计划必然撞
        # 「意图与计划不一致」；正道是自由循环——LLM 读文档直接讨论，要产出时
        # 循环内照样能编排。按合同模板注入灵感卡/历史附件/提示词规范。
        # 上下文合同 P1：任务会话登记——首链存原始任务句与材料，接续轮判定
        # 优先查登记（词表只兜底），登记缺失全部 fail-open 退回现行判定。
        trace = trace + ["💬 设计讨论 → 自由循环（不编机械计划）"]
        try:
            from app.services import task_session_store
            task_session_store.save({
        ...
        except Exception:  # noqa: BLE001 - 登记失败不阻断讨论
            pass
        return _run_fabric_content_loop(ctx, state, intent_text=intent_text,
                                        judge_text=_fabric_judge, text=text,
                                        trace=trace, output_dir=output_dir,
    ...
    if outcome.plan is None:
        if any("没有任何 submit/collect" in e for e in outcome.errors):
            # 2026-09-11 智能兜底：讨论/咨询型请求（「跟我讨论各服装的生图提示词」）
            # 会被规模词+生成词误带进机械计划——讨论本就不该有 submit 步骤，
            # 重试只会死循环撞同一堵墙。不再报错死路，降级自由循环（断点审批），
            # 让 LLM 直接读文档与用户讨论；真要出图时循环内照样能编排 submit。
            run_trace.emit(ctx, "plan.compile", status="fallback_fabric",
                           errors="; ".join(outcome.errors)[:300])
            trace = trace + ["🧭 意图与计划形态不符（疑似讨论型请求）→ 降级自由循环"]
            return _run_fabric_content_loop(ctx, state, intent_text=intent_text,
                                            judge_text=_fabric_judge, text=text,
                                            trace=trace, output_dir=output_dir,
                                            configured=configured)
...
def roleplay_node(state: AgentState) -> dict:
    ...
    try:
        ...
        # 世界书召回段（每轮按相关度重排）：同一套世界信息正则，但**落尾部动态块**
        # （history 之后，与数据表召回同位置）。若与锚点段拼在一起塞进世界书块（history 之前），
        # 它每轮一变就把其后的历史与全部预设片段打掉——真机实测相邻轮字节 LCP 78% → 62%。
        wb_recall = str(ctx.get("_worldbook_recall_text") or "")
        if wb_recall:
            wb_recall = _apply_regex(ctx, wb_recall, Placement.WORLD_INFO, is_prompt=True, depth=0)
            ctx["_worldbook_recall_text"] = wb_recall
        # 世界书动态段（2026-09-13 动态区外置）：锚点条目的【剧情进展·动态】区 + 会话中途
        # 新激活的锚点条目。与召回段同一落点（尾部动态块）——留在锚点段会把分叉点拉回
        # 世界书块内，真机三次 cached=0 的分叉点全在那里。
        wb_dynamic = str(ctx.get("_worldbook_dynamic_text") or "")
        if wb_dynamic:
            wb_dynamic = _apply_regex(ctx, wb_dynamic, Placement.WORLD_INFO, is_prompt=True, depth=0)
            ctx["_worldbook_dynamic_text"] = wb_dynamic
        run_trace.emit(
            ...
            dropped=int(ctx.get("_worldbook_dropped") or 0),
            recall_chars=len(wb_recall),
        )
        ...
        preset_msgs, preset_temp, has_hist, chains_tail, chains_head = _resolve_preset(
            ...
            history=story_round)
        # P4-roleplay（2026-09-12）：跨轮前缀复用的总前提——把「每轮变化」的块从 system 头
        # 移到尾部 system 消息。静态头（人设/角色卡/世界书/骰点规则/状态更新规则/文风）跨轮
        # 逐字节稳定；动态块（连续性状态/NPC 目标/RAG 召回/表格/directive）后置。
        # 两分支（内置 / 预设）同步拆；内容一字不删，只换位置（红线：位置可动、内容不可动）。
        ...
        # 世界书**召回段**（每轮重排）落这里：与数据表召回/连续性状态同层，都在 history 之后。
        # 锚点段（constant/keyword）留在世界书块内（history 之前）跨轮稳定 ⇒ 前缀不被召回打断。
        if wb_recall:
            dynamic_parts.append(wb_recall)
        # 世界书动态段（动态区+中途新锚点）：同样只落尾部动态块，理由同上。
        wb_dynamic = str(ctx.get("_worldbook_dynamic_text") or "")
        if wb_dynamic:
            dynamic_parts.append(wb_dynamic)
        ...
        if deps is not None:
            ...
            if compiled.text:
                dynamic_parts.append(compiled.text)
            if table_recall:
                dynamic_parts.append("【相关数据表行（独立配额）】\n" + table_recall)
            run_trace.emit(
                ctx, "continuity.compiled", tokens=compiled.tokens,
                sources=list(compiled.included), fact_count=len(active_facts),
            )
            # 搭车指令是固定文本（跨轮逐字节相同）→ 留静态头；本轮自主行动裁定 directive
            # 每轮变 → 后置（P4-roleplay）。
            base += roleplay_agency.state_instruction()
            if directive:
            ...
            try:
                ...
                if turn_tables:
                    dynamic_parts.append(table_update.table_context(turn_tables))
                    run_trace.emit(
        ...
        # 收口：状态块/纪要召回/裁定指令等在 substitute 之后才生成（且预设分支根本没替换
        # base），可能含 {{user}}/{{char}}（如状态字段名「对{{user}}态度」、快照）。统一在此对
        # 最终**两段**各替换一次，缺省 user 回退「我」（用户没填人设时也不让字面 {{user}} 漏进
        # system→被模型照抄进正文）。P4-roleplay：静态头与动态尾段分别替换，两段都不得漏。
        from app.services import preset_store as _ps2
        _macros = {
            "char_name": (ctx.get("card_name") or "").strip(),
            "user_name": (ctx.get("user_name") or "").strip(),
        }
        base = _ps2.substitute_macros(base, _macros)
        dynamic_text = _ps2.substitute_macros(
            "\n\n".join(part for part in dynamic_parts if part.strip()), _macros,
        )
        system = _agent_system(ctx, base)
        # 输出结构模板前置（2026-09-12 用户拍板方案 A + 顺序定稿）：首因位置守约率最高——
        # 模型守约率波动（21:11 写 2726 字思考 / 21:26 零思考）靠尾部纪律压不住，模板放
        # system 最开头给最强 few-shot。顺序=思考 → 状态栏 → 正文（用户定稿）；
        # 命定/登场/检定事件块按剧情穿插正文中间。解析层全部位置无关
        # （extract_status_snapshot 取末块、事件块就地解析），模板是行为引导不是解析契约。
        system = (
            "【输出结构（每轮固定，顺序不可换）】\n"
            "第一步 <think>完整推演思考</think>——按预设思考框架展开推演；\n"
            "第二步 <status>状态栏</status>——延续上轮格式逐字段更新；\n"
            "第三步 <content>正文</content>——按预设 <length> 字数要求足量输出；"
            "<roll>/<encounter>/<fate> 等事件块按剧情需要穿插在正文中间。\n"
            "所有标签必须成对完整闭合，各类块只输出一次，禁止在思考里预写任何标签草稿。\n\n"
            + system
        )
        # 尾部顺序（P4-roleplay）：[动态块][思维链 tail][输出纪律][…][本轮 user]。
        # 动态块是每轮变化的块，落在历史之后 → 静态头（system）逐字节跨轮稳定。
        tail_msgs: list[dict] = []
        if dynamic_text:
            tail_msgs.append({"role": "system", "content": dynamic_text})
        # 尾部思维链作独立 system 消息落在历史之后、本轮 user 之前 → 离生成点最近，遵守最严
        tail_msgs.extend({"role": "system", "content": c} for c in chains_tail)
        # 思考纪律（2026-09-12 用户拍板修订）：思考定位为**完整推演思考**——发布后由前端
        # 「小cot折叠」正则折叠为「推演内思考过程」下拉（用户设计，思考本就该发布而非丢弃）。
        # 旧版「三类内容 + 300 字硬上限」与预设「默认COT组装」框架直接矛盾，模型面对矛盾
        # 指令干脆不写思考（09-12 20:32 trace 实证思考=0）。保留防烧钱核心禁令
        # （试写正文/复述设定/推翻重来/标签草稿——2026-08-29/30 事故教训），删思考长度压制；
        # 正文长度以预设 <length> 字数区间为准（预设已启用 1000-8000），不在后端另设硬指标。
        # 思考纪律（2026-09-12 三次修订定稿）：思考定位为**完整推演思考**（发布后由前端
        # 「小cot折叠」折叠为「推演内思考过程」下拉）。历次实验：300 字硬上限 → 模型干脆
        # 不写思考（deepseek）；点名禁令清单 → 冗余根因其实在预设思考框架本身（用户裁定
        # 禁令去掉，去预设里找）。定稿=只保留**思考预算原则**：思考总篇幅 ≤ 正文一半
        # （软比例，用户认可），预算外的冗余治理落在预设思考框架的瘦身（另行处理）。
        # 泄漏修订（2026-09-13）：09-12 版把「前端折叠」「1000-8000 字」等实现细节写进了
        # 模型可见文本，模型照抄进正文（神权大陆 msg[4] 实证：质量检查/状态栏更新/
        # 【系统提示…尾部标注】三处泄漏）。净提示词=删全部实现细节 + 追加元话语硬禁令；
        # 出口兜底见 roleplay_turn.strip_meta_leak_in_content（只动 <content>）。
        tail_msgs.append({
            "role": "system",
            "content": (
                "[输出纪律] 严格按 system 开头的【输出结构】执行：①<think>完整推演思考</think>"
                "——按预设思考框架展开推演，写成连贯的多段推演，禁止写成一句话摘要糊弄；\n"
                "②<status>状态栏</status>——在思考之后、正文之前，延续上轮格式逐字段更新；\n"
                "③正文——按预设 <length> 的字数区间足量交付，"
                "不因思考延长而缩水正文；<roll>/<encounter>/<fate> 等事件块穿插正文中间。\n"
                "思考预算原则：思考的每个字都按输出计费——总篇幅**不超过正文的一半**。"
                "推演的价值在决策质量不在铺陈，把输出预算留给正文、状态栏与事件块。\n"
                "硬禁令：只输出剧情本体。禁止在任何输出区写面向系统的说明文字——包括但不限于"
                "质量检查、字数统计/字数要求、对状态栏的更新说明、【系统提示】/【系统说明】等"
                "标注块。这类文字不是剧情，一个字都不许出现。"
            ),
        })
...
def _agency_writeback(ctx: dict, deps, reply: str, turn: int, affinity,
    ...
    try:
        ...
        # 提取链只看正文，发布时拼回前缀。
        think_head, body = roleplay_agency.split_think_prefix(reply)
        # 孤立 </think>（模型漏写 <think> 开标签，2026-09-12 实锤）会随正文发布，
        # 发布链先剥；成对块与未闭合开标签不受影响。规划注释情形由裸正文兜底路径
        # （_wrap_bare_body → repair_orphan_think_close）补包 think 处理。
        body = roleplay_agency.strip_orphan_think_close(body)
...
def _worldbook_session_state_path(ctx: dict) -> str:
    """世界书稳定序列的持久化文件路径（2026-09-13 动态区外置配套）。

    与 repo 快照同目录（`<output_dir>/<safe repo_id>/`，见 worldbook_store.repo_snapshot_path），
    重启后锚点段不重排、前缀跨进程延续。取不到 output_dir/repo_id 时返回空串 → 不落盘。
    """
    from pathlib import Path

    from app.services.pathnames import safe_seg
    base = str(ctx.get("output_dir") or "").strip()
    repo_id = str(ctx.get("repo_id") or ctx.get("thread_id") or "").strip()
    if not (base and repo_id):
        return ""
    return str(Path(base) / safe_seg(repo_id, strip=False) / "worldbook_session.json")


def _worldbook_session_key(ctx: dict) -> str:
    """世界书稳定序列的会话键（2026-09-12，P4-roleplay 配套）。

    取「作品」粒度（output_dir + repo_id/thread_id）：同一作品连续推进的剧情共享一条序列，
    换作品/换线程各自独立。取不到任何一项时返回空串 → 调用方退回无状态的重排版
    assemble_selection（不引入跨轮状态）。
    """
    parts = [
        str(ctx.get("output_dir") or "").strip(),
        str(ctx.get("repo_id") or ctx.get("thread_id") or "").strip(),
    ]
...
def _resolve_worldbook(ctx: dict, query: str) -> str:
    ...
    # 召回段（每轮重排）单独外带：调用方把它放尾部动态块，避免它一变就把其后整段前缀打掉。
    ctx["_worldbook_recall_text"] = ""
    # 动态段（动态区外置）同样外带：锚点条目【剧情进展·动态】区 + 会话中途新锚点。
    ctx["_worldbook_dynamic_text"] = ""
    try:
        ...
        # 会话内稳定序列（2026-09-12）：条目按首次激活顺序单调追加 → 注入文本跨轮只增不改，
        # 上游前缀缓存才能命中（重排版每轮整块换序，位于其后的历史与尾部合同全部作废）。
        # 取不到会话键时退回无状态的重排版，行为与改动前一致。
        session_key = _worldbook_session_key(ctx)
        selection = (
            worldbook.assemble_selection_stable(
                ctx.get("repo_id", ""), entries, scan, cfg, session_key=session_key,
                state_path=_worldbook_session_state_path(ctx) or None,
            )
            if session_key
            else worldbook.assemble_selection(ctx.get("repo_id", ""), entries, scan, cfg)
        )
        ctx["_selected_worldbook_indices"] = selection.indices
        ctx["_worldbook_dropped"] = selection.dropped
        ...
        ctx["_worldbook_recall_text"] = selection.recall_text
        ctx["_worldbook_dynamic_text"] = getattr(selection, "dynamic_text", "")
...
def _history_summarizer(context: RunContext):
    """自动压缩的摘要回调（/compact 对齐）：LLM 把旧段历史压成结构化要点。

    失败/空摘要返回 None → recent_history 退回机械保头保尾裁剪（fail-open）。
    """
    try:
        base = context.chat.base_url
        key = context.chat.api_key
        model = context.chat.model
    except Exception:  # noqa: BLE001 - dict ctx/缺 chat 配置 → 无摘要器（机械路径）
        return None
    if not (base and model):
        return None
    proxy = (context.chat_proxy_url or "").strip()

    def _summarize(old: list[dict]) -> str | None:
        transcript = "\n".join(
            ("用户" if item.get("role") == "user" else "助手") + "：" + str(item.get("content") or "")
            for item in old)
        system = (
            "你是对话压缩器。把以下历史对话压缩成一段结构化摘要，供后续对话作为唯一历史依据。"
            "必须保留：任务目标与演变、已确认的决定与设定（角色/服装/配色/文件路径等专名与数字原样保留）、"
            "用户明确否决的内容、未完成事项。不新增内容、不评论、不提问。只输出摘要正文。")
        try:
            out = _llm.chat(base, key, model, system, transcript, temperature=0.2,
                            **({"proxy": proxy} if proxy else {}))
        except Exception:  # noqa: BLE001 - 压缩失败退机械裁剪
            return None
...
def stream_multi_agent(context: RunContext) -> Iterator[dict]:
    ...
    # 2026-09-11 上下文 token 化：有效预算 = min(设置上限, 模型窗口−输出预留)，
    # 条数约束废弃（剧情模式历史走 last_story_round，不受影响）。达到触发
    # 百分比自动压缩（LLM 摘要旧段，失败退机械保头保尾），对齐市面 /compact。
    _hist_budget = agent_context.resolve_history_budget(
        context.context_max_tokens, context.preset_sampling)
    context.history = agent_context.recent_history(
        context.thread_id,
        max_tokens=_hist_budget,
        per_role=0,
        history_override=context.history_override,
        compact_trigger_pct=context.compact_trigger_pct,
        summarize_fn=_history_summarizer(context),
    )
