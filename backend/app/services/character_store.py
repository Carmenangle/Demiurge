...
CHAT_FILE = "chat.json"
# 卡目录归属标记（2026-09-09 治本）：记录该卡属于哪个作品 repo_id，供
# character.embed_worldbook / workspace.export_to_library 只处理本作品的卡，
# 防「作品库根下多作品混放」时把当前世界书写进别的作品的卡。
# 与 repo_meta 的 _repo.json（仓库文件夹标记）语义不同，故意不复用文件名。
WORK_MARKER_FILE = "_work.json"
...
def write_work_marker(base: str, name: str, repo_id: str) -> None:
    """写卡目录归属标记（<卡目录>/_work.json）。空 repo_id → 不写。"""
    if not ((base or "").strip() and (name or "").strip() and (repo_id or "").strip()):
        return
    folder = card_dir(base, name)
    folder.mkdir(parents=True, exist_ok=True)
    (folder / WORK_MARKER_FILE).write_text(
        json.dumps({"repo_id": str(repo_id)}, ensure_ascii=False), encoding="utf-8")


def read_work_marker(base: str, name: str) -> str:
    """读卡目录归属的 repo_id；无标记/损坏 → 空串。"""
    p = card_dir(base, name) / WORK_MARKER_FILE
    if not p.is_file():
        return ""
    try:
        ...
        return ""
    return str((data or {}).get("repo_id") or "") if isinstance(data, dict) else ""


def owned_card_dirs(base: str, repo_id: str) -> list[Path]:
    """**归属判定单一属主**：返回本作品拥有的卡目录（库根一层），绝不误伤其它作品。

    卡一律落 `<base>/<卡名>/`（`character.upsert_repo` 的 base 注入是**作品库根**），
    而卡目录名与作品文件夹名可以不同（实锤：仓库「原创」的卡在 `<base>/御仙/`）。
    归属判定顺序（2026-09-09 治本，2026-09-10 B3/A 收口到本函数）：
    ① 卡目录 `_work.json` 的 repo_id 匹配（落卡能力写入；固化03「卡名≠作品名」靠它）；
    ② 卡目录名 == 仓库名（固化02「卡名=作品名」的既有约定）；
    都不匹配 → 跳过（宁可不动，也不污染）。

    调用方：`capability_handlers._work_card_files`（内嵌/导出只处理本作品的卡）、
    `repo_meta.artifact_domains`（产物访问/收集域）。
    """
    from app.services import repo_meta

    if not ((base or "").strip() and (repo_id or "").strip()):
        return []
    root = Path(base)
    if not root.is_dir():
        return []
    work_name = repo_meta.repo_name(repo_id)
    out: list[Path] = []
    try:
        cards = sorted(root.glob("*/card.json"))
    except OSError:
        return []
    for card_file in cards:
        folder = card_file.parent
        if folder.name in ("_prep", "_versions"):
            continue
        owner = read_work_marker(str(root), folder.name)
        if owner:
            if owner == repo_id:
                out.append(folder)
            continue
        if work_name and folder.name == work_name:
            out.append(folder)
    return out
