...
# 中断半成品落盘后缀（唯一属主，三处消费方对齐）：
#   - generation_store.persist_text(interrupted=True)    → 快照文本拼该后缀（刷新后仍可识别）
#   - chat_memory.append_turn(interrupted=True)          → checkpoint 文本拼该后缀
#   - agent_graph._resume_interrupted_messages           → 下一轮靠「尾部是它」触发断点续写
# 改动标记必须三处同步，否则跨轮续写识别静默失效（2026-09-04 收口实锤）。
INTERRUPT_MARKER = "（已打断）"
...
def edit_message(thread_id: str, message: dict) -> bool:
    """按 id 原子替换**既有**消息正文（用户编辑落库唯一通道）；目标不存在不写入。

    与 upsert（生成链追加用）分离：编辑永远不该制造幽灵消息。返回是否命中。
    2026-09-13 实锤：编辑只靠全量快照保存（防抖可静默失败），失败后下一轮生成
    从旧快照取历史把编辑打回——用户删掉的「（正文到此…）」在新生成后重新出现。
    """
    mid = message.get("id")
    if not mid:
        return False
