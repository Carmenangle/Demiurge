...
def _keep_budget_lines(keeps: list[tuple[str, str]], budget: int,
                       summarizer: Any) -> list[str]:
    """keep 档 token 预算（P2b，2026-09-12）：同域历史全量保留在长讨论里无限膨胀。

    超预算时：近期一半预算原文保留；更早部分优先交 LLM 压成「任务进度摘要」
    （定了什么/改了什么/待定什么，对齐 auto-compact），失败退机械聚合标注
    （_aggregate_head，零 LLM）。budget<=0 或不足 4 条 → 原样（旧行为）。
    summarizer 签名对齐 agent_context：收 [{role,content}]，回摘要文本或 None。
    """
    from app.services import agent_context as _ac
    if budget <= 0 or len(keeps) < 4:
        return [f"[相关·保留] {role}：{text}" for role, text in keeps]
    costs = [_ac.estimate_tokens(text) for _, text in keeps]
    if sum(costs) <= budget:
        return [f"[相关·保留] {role}：{text}" for role, text in keeps]
    tail_budget = max(1, budget // 2)
    tail_cost = 0
    split = 0
    for i in range(len(keeps) - 1, -1, -1):
        if tail_cost + costs[i] > tail_budget:
            split = i + 1
            break
        tail_cost += costs[i]
        split = i
    if split <= 0 or split >= len(keeps):
        return [f"[相关·保留] {role}：{text}" for role, text in keeps]
    old, tail = keeps[:split], keeps[split:]
    summary = ""
    if summarizer is not None:
        try:
            summary = (summarizer([{"role": "user" if role == "用户" else "assistant",
                                    "content": text} for role, text in old]) or "").strip()
        except Exception:  # noqa: BLE001 - 摘要失败退机械聚合
            summary = ""
    lines = []
    if summary:
        lines.append(f"[任务进度摘要（更早讨论的自动压缩：任务目标/已确认决定/否决项/待办）] {summary}")
    else:
        lines.append(f"[相关·聚合] 此前同域讨论 ×{len(old)}"
                     f"（涉及：{_aggregate_head([text for _, text in old])}），细节从略")
    lines += [f"[相关·保留] {role}：{text}" for role, text in tail]
    return lines
...
def history_text_gated(
    ...
    *,
    ...
    keep_token_budget: int = 0,
    keep_summarizer: Any = None,
) -> str:
    ...
    P2b（2026-09-12）：keep_token_budget>0 时 keep 档按 token 预算收口——超限旧段
    LLM 压成任务进度摘要（失败退机械聚合），近期原文保留（_keep_budget_lines）。
    ...
    lines.extend(_keep_budget_lines(keeps, keep_token_budget, keep_summarizer))
