...
class ChronicleEntry:
    ...
    seq: int = 0           # 展示序号（派生值，不落库）：同层按回合排第几条，由 assign_sequence 填

    def card_id(self) -> str:
        """稳定展示编号：T<层级>-<序号>。

        2026-09-13 用户定案：编号是「在故事里的第几条」，不是落盘 rowid。rowid 只是插入
        序——补跑历史层/整理后它会与回合序脱钩（实测最早区间 [1–3] 显示成 T1-13）。有
        序号（读路径 assign_sequence 填）时按序号；纯逻辑构造（写路径）退回 rowid。
        """
        if self.seq > 0:
            return f"T{self.layer + 1}-{self.seq}"
...
def assign_sequence(entries: list[ChronicleEntry]) -> None:
    """给每层条目按回合先后分配展示序号（1 起），原地写 entry.seq（纯逻辑）。

    2026-09-13 用户定案：卡号必须是「故事里的第几条」。落盘 rowid 只是插入序——
    补跑历史层 / 整理封口后它会与回合序脱钩（实测最早区间 [1–3] 被记为 T1-13）。
    同层内按 (turn_start, turn_end, rowid) 排序后 1..n 连续编号：缺口不占号，
    整理后首条即 T<层>-1（用户要求「弥补空缺」）。
    """
    counters: dict[int, int] = {}
    for entry in sorted(entries, key=lambda e: (e.layer, e.turn_start, e.turn_end, e.rowid)):
        counters[entry.layer] = counters.get(entry.layer, 0) + 1
        entry.seq = counters[entry.layer]
...
# ── 区间规范化（2026-09-13 用户定案）：把重叠的回合区间整理成「每回合至多一条」──
# 用户实锤三类缺陷：①覆盖重跑让同一区间存多条（manual_table_fill 的 overwrite 分支已改为
# 不删旧条，重复从此无法避免）；②相邻批次共享边界层（33-35 与 35-37 把第 35 层记两次）；
# ③rowid 是插入序而非回合序，补跑历史层后 recent() 会把旧剧情当最新注入。
# 本组函数只做**纯计算**（entries → 计划），落盘在 narrative_store。
# 定案：边界重叠「顺延保宽」（36-37 → 36-38，不截断信息密度）；重复组由前端询问是否覆盖。


@dataclass
class DuplicateGroup:
    """同一回合区间存在多条纪要（覆盖重跑 / 重复补漏产生）。

    rowids 升序；默认保留最后一条（rowid 最大 = 最近一次生成，即「覆盖」语义），
    其余让位。前端据此逐组询问用户是否覆盖。
    """
...
class RangeRebase:
    ...
    用户定案「顺延保宽」：起点前移到前一条终点之后，终点同步后移以保持原宽度
    （33-35 / 35-37 → 33-35 / 36-38），该条纪要的信息密度不变。
    """
...
@dataclass
class NormalizePlan:
    """区间规范化计划（只读产物，落盘前供用户确认）。"""

    total_turns: int
    duplicates: list[DuplicateGroup] = field(default_factory=list)
    rebases: list[RangeRebase] = field(default_factory=list)
    gaps: list[tuple[int, int]] = field(default_factory=list)
    covered_turns: int = 0
    overrun: list[int] = field(default_factory=list)   # 顺延后超出 total_turns 的 rowid

    @property
    def clean(self) -> bool:
        """无重复、无边界重叠 → 无需整理。"""
        return not (self.duplicates or self.rebases)


def coverage_index(entries: list[ChronicleEntry], *, total_turns: int = 0) -> dict[int, list[int]]:
    """每回合被哪些纪要覆盖：turn -> [rowid, ...]（升序）。

    用户要求「每条消息的表格次数要计数」——列表长度 > 1 即该回合被记了多次，
    是区间整理（去重 / 顺延）的判据来源。
    """
    index: dict[int, list[int]] = {}
    for entry in entries:
        start = max(1, entry.turn_start)
        end = max(start, entry.turn_end)
        if total_turns > 0:
            end = min(end, total_turns)
        if end < start:
            continue
        for turn in range(start, end + 1):
            index.setdefault(turn, []).append(entry.rowid)
    return index


def coverage_counts(entries: list[ChronicleEntry], *, total_turns: int = 0) -> dict[int, int]:
    """每回合被多少条纪要覆盖（重复检测的基础口径）。"""
    counts: dict[int, int] = {}
    for entry in entries:
        start = max(1, entry.turn_start)
        end = max(start, entry.turn_end)
...
def compress_ranges(turns: set[int]) -> list[tuple[int, int]]:
    """离散回合集合 → 连续区间列表（升序）。"""
    if not turns:
        return []
...
def normalize_plan(entries: list[ChronicleEntry], *, total_turns: int = 0) -> NormalizePlan:
    """把重叠的纪要区间整理成「每回合至多一条」的计划（纯计算，不落盘）。

    两步，顺序不可换：
    ① **同区间多条归组**——完全同区间的条目判为重复（覆盖重跑的产物），保留 rowid 最大者
       （最近一次生成），其余让位。必须先做这步，否则重复条目会被 ② 推到未来回合。
    ② **相邻边界顺延**——剩者按回合升序扫描，与前一条共享边界层时保持宽度后移
       （用户定案 36-37 → 36-38），不截断内容密度。
    """
    live = [
        entry for entry in entries
        if entry.rowid > 0 and entry.turn_start >= 1 and entry.turn_end >= entry.turn_start
        and (not total_turns or entry.turn_start <= total_turns)
    ]
    buckets: dict[tuple[int, int], list[int]] = {}
    for entry in live:
        buckets.setdefault((entry.turn_start, entry.turn_end), []).append(entry.rowid)
    duplicates = [
        DuplicateGroup(start, end, sorted(ids))
        for (start, end), ids in sorted(buckets.items()) if len(ids) > 1
    ]
    surrendered = {rid for group in duplicates for rid in group.drop}
    kept = sorted(
        (entry for entry in live if entry.rowid not in surrendered),
        key=lambda entry: (entry.turn_start, entry.turn_end, entry.rowid),
    )
    rebases: list[RangeRebase] = []
    finals: dict[int, tuple[int, int]] = {}
    cursor = 0
    for entry in kept:
        if entry.turn_start > cursor:
            finals[entry.rowid] = (entry.turn_start, entry.turn_end)
            cursor = entry.turn_end
            continue
        width = max(0, entry.turn_end - entry.turn_start)
        new_start = cursor + 1
        new_end = new_start + width
        rebases.append(RangeRebase(
            rowid=entry.rowid, old_start=entry.turn_start, old_end=entry.turn_end,
            new_start=new_start, new_end=new_end,
        ))
        finals[entry.rowid] = (new_start, new_end)
        cursor = new_end
    covered: set[int] = set()
    for start, end in finals.values():
        covered.update(range(max(1, start), end + 1))
    if total_turns > 0:
        covered = {turn for turn in covered if turn <= total_turns}
    overrun = sorted(
        rid for rid, (_, end) in finals.items() if total_turns > 0 and end > total_turns)
    gaps = compress_ranges(set(range(1, total_turns + 1)) - covered) if total_turns > 0 else []
    return NormalizePlan(
        total_turns=total_turns, duplicates=duplicates, rebases=rebases,
        gaps=gaps, covered_turns=len(covered), overrun=overrun,
    )
