...
def submit_task(plan: GenerationPlan, *, output_dir: str, repo_id: str = "",
    ...
    for step in plan.steps:
        # 落盘/执行类参数一律环境归一（模型不得决定写入外部）。清单单一属主 =
        # capability_registry.ENV_INJECTED_OPS：新增能力只改注册表一处，不再逐处漏加
        # （2026-09-09 实锤：character.embed_worldbook 漏加 → 执行时缺 base/repo_id）。
        # current_flow_doc（固化链续跑 §3 设计 A）：doc.create_repo 的 repo_id 同样由
        # 环境注入，模型只需给 rel_path/content。
        capability_registry.inject_env_params(
            step.params, step.operation,
            output_dir=output_dir, repo_id=repo_id or plan.repo_id, force=True)
...
def approve_task(task_id: str, *, approved_by: str = "user") -> dict:
    ...
    with get_connection() as connection:
        connection.execute(
            ...
            (lease["id"], _now(), task_id))
    _publish_progress(task_id, "queued")
    _wake()
...
def _requeue(task_id: str) -> None:
    with get_connection() as connection:
        changed = connection.execute(
            "update plan_tasks set status='queued', error='', updated_at=? where id=? "
            "and status != 'running'",
...
def _claim_next():
    ...
    with get_connection() as connection:
        ...
        changed = connection.execute(
            """
            update plan_tasks set status='running', worker_id=?, lease_expires_at=?,
            error='', updated_at=?
...
def _run_task(task, cancel: threading.Event) -> None:
    ...
    for seq, step in enumerate(steps):
        ...
        # 执行层兜底（2026-09-09 治本）：按 handler 真实签名补齐环境注入参数。
        # 计划可能来自编译、配方重放或 fabric 自由循环，任一来源漏注入都在这里补上，
        # 避免「计划能编译、执行时 TypeError」——实锤 character.embed_worldbook。
        params.update(_env_params_for_handler(
            operation, params, output_dir=output_dir, repo_id=str(task["repo_id"])))
...
def _env_params_for_handler(operation: str, params: dict, *,
                            output_dir: str, repo_id: str) -> dict[str, str]:
    """按 handler 真实签名补齐环境注入参数（执行层最后一道防线）。

    只补「handler 必需、params 里没有、名字在 ENV_PARAM_SOURCES 里」的位置参数；
    值取运行环境（base/output_dir/cwd ← 作品根，repo_id ← 运行会话）。
    签名取不到（未注册/import 失败）→ 返回空，不阻断执行。
    """
    cap = capability_registry.get(operation)
    if cap is None:
        return {}
    ...
    except Exception:  # noqa: BLE001 - 签名不可得时不猜参数
        return {}
    missing: dict[str, str] = {}
    for name, param in signature.parameters.items():
        if name in params or name not in capability_registry.ENV_PARAM_SOURCES:
            continue
        if param.kind in (inspect.Parameter.VAR_POSITIONAL, inspect.Parameter.VAR_KEYWORD):
            continue
        if param.default is not inspect.Parameter.empty:
            continue
        value = capability_registry.env_param_value(
            name, output_dir=output_dir, repo_id=repo_id)
        if value:
            missing[name] = value
    return missing
    ...
    with get_connection() as connection:
        ...
        if current_status not in ("blocked", "awaiting_approval"):
            if status in ("done", "cancelled"):
                # 2026-09-09 实锤：重试成功后 status=done 但 error 仍留着旧的
                # needs_approval（前端显示「完成却报错」）。终态成功/取消必须清空 error；
                # partial 保留（最后失败原因是有效诊断）。
                connection.execute(
                    "update plan_tasks set status=?, error='', worker_id='', lease_expires_at=0, "
                    "updated_at=? where id=?", (status, _now(), task_id))
            else:
                connection.execute(
