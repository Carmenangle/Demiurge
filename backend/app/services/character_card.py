...
def _make_png_text_chunk(keyword: bytes, text: str) -> bytes:
    """构造 PNG tEXt chunk：[4B length][tEXt][keyword + NUL + text][4B CRC]"""
    import zlib
    data = keyword + b"\x00" + text.encode("latin-1", "replace")
    return (struct.pack(">I", len(data)) + b"tEXt" + data
            + struct.pack(">I", zlib.crc32(b"tEXt" + data) & 0xFFFFFFFF))


def _blank_png() -> bytes:
    """最小合法 PNG（1x1 透明 RGBA），作为无原图导出时的载体画布。"""
    import zlib
...
def _strip_card_text_chunks(image: bytes) -> bytes:
    """剥离原图已有的 ccv3/chara tEXt chunk（避免与新的导出数据重叠，
    读端 ccv3 后覆盖前导致 roundtrip 读到旧卡——2026-09-06 实锤）。"""
    out = bytearray(image[:8])
    pos = 8
    n = len(image)
    while pos + 8 <= n:
        (length,) = struct.unpack(">I", image[pos:pos + 4])
        ...
        if data_end > n:
            break
        if ctype == b"tEXt" and length > 0:
            sep = image[pos + 8:data_end].find(b"\x00")
            kw = (image[pos + 8:pos + 8 + sep].decode("latin-1", "replace").lower()
                  if sep != -1 else "")
            if kw in ("ccv3", "chara"):
                pos = data_end + 4
                continue
        out += image[pos:data_end + 4]
        pos = data_end + 4
        if ctype == b"IEND":
            break
    return bytes(out)
...
def build_png_card(card: dict[str, Any], avatar: bytes | None = None) -> bytes:
    """把角色卡 JSON 打包进 PNG tEXt chunk（ccv3 优先，兼容读端），返回 PNG 字节。

    avatar 给定时以其为画布（保留原图作为卡头像，并剥离原图旧卡数据）；
    否则生成最小 1x1 PNG 载体。与 read_png_card_json 互为逆操作（roundtrip 一致）。
    """
    raw = json.dumps(card, ensure_ascii=False, separators=(",", ":"))
    encoded = base64.b64encode(raw.encode("utf-8")).decode("ascii")
    text_chunk = _make_png_text_chunk(b"ccv3", encoded)
    if avatar and avatar.startswith(PNG_SIGNATURE):
        base = bytearray(_strip_card_text_chunks(avatar))
    else:
        base = bytearray(_blank_png())
    # 在 IHDR chunk 之后（签名 8B + IHDR chunk 25B）插入 tEXt
    insert_at = 8 + 4 + 4 + 13 + 4
    return bytes(base[:insert_at]) + text_chunk + bytes(base[insert_at:])
