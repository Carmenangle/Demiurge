...
def encode_event(event: Mapping[str, object]) -> ChatStreamEvent | None:
    ...
    for key in ("trace", "delta", "thinking", "replace", "route", "image", "video",
                ...
                "insp", "rag_status", "approval", "route_choice", "artifacts", "error"):
    ...
    kind = signals[0]
    if kind in ("delta", "thinking", "replace"):
        return _wire(kind, {"text": str(event[kind])})
    if kind == "trace":
        data = {"text": str(event["trace"])}
        # 2026-09-06：trace 可携带执行详情（思考文本/工具参数/结果摘要），
        # 前端执行过程面板点开「思考/工具」面板查看；无 detail 不携带（旧前端宽松兼容）。
        detail = event.get("detail")
        if isinstance(detail, str) and detail:
            data["detail"] = detail
        return _wire("trace", data)
    if kind == "route":
        return _wire("route", {"route": str(event["route"])})
    if kind == "artifacts":
        items = event["artifacts"]
        if not isinstance(items, list):
            raise ValueError("artifacts 事件必须是列表")
        clean: list[dict] = []
        for item in items:
        ...
        return _wire("artifacts", {"items": clean})
