...
# ST 脚本变量（2026-09-04 事故实锤）：预设作者用 {{setvar::name::value}} 定义「思维链格式/
# 卡思维链/ds思维链」等思考引导变量，值由 ST 前端按通道消费。Demiurge 无 ST 前端 → 定义块
# 原样泄漏进 system，与 XML <think>/<content> 输出协议打架（GrayWill prompts[3]「ds启用.新」
# 把思维链格式定义为 ds 原生 thinking/response 裸格式 → deepseek 通道整段裸写正文、
# ensure opened=0 判死、3 次无谓重掷白烧）。渲染时实现 ST 语义子集：
#   {{setvar::name::value}}   注册 name=value，自身输出空；
#   {{getvar::name}}          展开为已注册值（未注册 → 空串）；
#   {{name}}（已注册）         同上裸引用；未注册保留原样（{{char}}/{{user}} 等宏先于本层替换）。
# 同名后定义覆盖先定义；值内立即展开当时已注册引用（链式定义，先定义先引用）。
_GETVAR_REF_RE = re.compile(r"\{\{getvar::([^{}]+?)\}\}", re.DOTALL)
# 裸引用 name 不含冒号（避开 setvar::/getvar:: 形态残留）
_BARE_REF_RE = re.compile(r"\{\{([^{}:]+?)\}\}")


def expand_setvar(content: str, scope: dict[str, str]) -> str:
    """渲染预设片段里的 ST setvar 语义；scope 在调用方遍历 prompt_order 时跨片段累积。"""
    if not content or "{{" not in content:
        return content
    out = _expand_setvar_defs(content, scope)
    out = _GETVAR_REF_RE.sub(lambda m: scope.get(m.group(1).strip(), ""), out)
    return _BARE_REF_RE.sub(lambda m: scope.get(m.group(1), m.group(0)), out)


_SETVAR_TAG = "{{setvar::"


def _expand_setvar_defs(text: str, scope: dict[str, str]) -> str:
    """逐段扫描 {{setvar::name::value}} 定义：注册（值内展开当时已注册引用）并输出空。

    值域用配平扫描而非正则非贪婪——值内可含嵌套 {{引用}}（如 value 引用更早定义的变量），
    非贪婪会在内层首个 }} 截断（smoke 实证 b='X{{a' 残片）。
    """
    out: list[str] = []
    i = 0
    n = len(text)
    while True:
        pos = text.find(_SETVAR_TAG, i)
        if pos < 0:
            out.append(text[i:])
            break
        out.append(text[i:pos])
        name_start = pos + len(_SETVAR_TAG)
        ...
        if not closed:
            out.append(text[pos:])
            break
        scope[name] = _expand_registered("".join(value), scope)
        i = k + 2
    return "".join(out)


def _expand_registered(text: str, scope: dict[str, str]) -> str:
    """展开 getvar 与已注册裸引用（未注册保留原样；不做嵌套 setvar 注册）。"""
    if "{{" not in text:
        return text
    out = _GETVAR_REF_RE.sub(lambda m: scope.get(m.group(1).strip(), ""), text)
    return _BARE_REF_RE.sub(lambda m: scope.get(m.group(1), m.group(0)), out)
...
# 内容宏（ST 惯例）：这些是**内容**，不是未解析占位符——世界观/角色卡里主角名就存成 `{{user}}`
# （`character_store` 与 `capability_handlers` 按用户定案主动写入），固化04 合并产物必然带它。
# **单一属主**：`substitute_macros` 的替换分支与 `plan_validator` 的占位符豁免都读这里；
# 新增内容宏只改这一处（大小写不敏感，ST 宏惯例）。
CONTENT_MACRO_NAMES: frozenset[str] = frozenset(
    {"char", "user", "lastusermessage", "lastcharmessage"})

# 形如 {{name}} / {{ name }} 的整token（token 只能是标识符，故 {{step1.output}} 不匹配）
_CONTENT_MACRO_RE = re.compile(r"\{\{\s*([A-Za-z_][A-Za-z0-9_]*)\s*\}\}")


def strip_content_macros(text: str) -> str:
    """剥掉登记的内容宏（`CONTENT_MACRO_NAMES`，大小写不敏感）；未登记的 `{{...}}` 原样保留。

    供计划参数校验判定「是否还有未解析占位符」：先剥登记宏，再看残留里是否仍有 `{{`/`}}`
    （见 `plan_validator._check_placeholders`）。于是 `{{user}}`（内容）放行，而
    `{{step1.output}}`（引用前序步骤）仍被拒。
    """
    if not text:
        return text
    return _CONTENT_MACRO_RE.sub(
        lambda m: "" if m.group(1).lower() in CONTENT_MACRO_NAMES else m.group(0), text)
...
def assemble_system(
    ...
    parts: list[str] = []
    scope: dict[str, str] = {}  # ST setvar 作用域：跨片段累积（2026-09-04 事故修复）
    for entry in order:
        ...
        if p.get("marker"):
            ...
            content = expand_setvar(content, scope)
            if content:
...
def sampling_params(preset: dict[str, Any]) -> dict[str, Any]:
    ...
    ⚠️ 2026-09-04 设置整合后已废弃：智能体参数预设（采样/输入/输出上限）权威源
    迁到 settings（`ctx.preset_sampling`，来自前端 payload）。本函数仅在以下场景保留：
    1) trace 事件里 temperature 字段的兼容回退（agent_graph._resolve_roleplay_inputs）；
    2) 旧 settings 迁移期的占位。**新代码请直接读 `ctx.preset_sampling`**，勿再依赖本函数。
    """
    out: dict[str, Any] = {}
