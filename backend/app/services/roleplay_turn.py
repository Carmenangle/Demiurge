...
# 裸思考混排标记：整段输出里出现这些字样 = 模型把 md 思考样式直接写进输出（无 <think> XML
# 包装）——思考文字不得发布成正文，这类输出不兜底（交回自愈/重掷）。
_BARE_THINK_MARKERS = re.compile(
    r"thinking\*|灰魂吐槽|输入分析|认知情境|关键NPC分析|欲望链|输出格式|推演",
    re.IGNORECASE,
)
# 剥壳时顺带清掉独立状态/事件块（裸输出不该有，防御残留）
_BARE_BLOCK_TAGS = re.compile(
    r"</?\s*(?:status|roll|encounter|illustration|表格更新|状态更新)\b[^>]*>",
    re.IGNORECASE,
)


def _wrap_bare_body(reply: str) -> str:
    """整段裸正文兜底：无任何 XML 标签的完整正文 → 自动包 <content> 通过结构校验发布。
    ...
    - 无裸思考混排标记（思考文字不该发布成正文）；
    - 剥 think/状态块后正文 ≥ CONTINUATION_MIN_BODY_CHARS（防协议字面量等短残片被包）；
    - 包装后过 ensure 结构校验。
    返回包装串；不适用返回空串。
    """
    raw = reply or ""
    ...
    if re.search(r"<content\b|<think\b", raw, re.IGNORECASE):
        return ""
    if _BARE_THINK_MARKERS.search(raw):
        return ""
    visible = _strip_think(raw).strip()
    if not visible:
        return ""
    body = _BARE_BLOCK_TAGS.sub("", visible).strip()
