...
def _compress_read_chunks(messages: list[dict]) -> None:
    ...
    0-220000/241709」，需要细节可再 read_text 重读指定段。幂等：已压缩的不重复压缩。
    """
    ...
    for path, items in by_path.items():
        ...
        for ai_idx, us_idx, offset, raw in items:
            if offset == max_offset:
                continue  # 最新一卷保留原文
            if _READ_SUMMARY_MARK in raw:
                continue  # 幂等：已压缩
            chars = 0
            total = ""
            try:
            ...
            messages[us_idx]["content"] = json.dumps(
                {"tool_result": summary}, ensure_ascii=False)
...
def _compress_upsert_results(messages: list[dict]) -> None:
    ...
    for idx, (ai_idx, comments) in enumerate(ids):
        ...
        try:
            ...
            continue
        params["entries"] = "UPSERT_ENTRIES 已写 " + str(count) + " 条：" + comments +             "（原文已压缩；需要细节可查看作品 worldbook.json 快照）"
        messages[ai_idx]["content"] = json.dumps(
...
def run_loop(*, intent: str, history: str = "", capabilities: list[dict] | None = None,
    ...
    _system_origin = system  # 进度卡每次决策前动态拼到 system（2026-09-06）
    if images:
    ...
    for step_index in range(1, max_steps + 1):
        ...
        # 动态拼进 system——模型长期自由循环几十步后容易迷失任务（实锤：固化02 转卡
        # 中途被历史生图带偏去 load 固化01/list_templates/lora.resolve）。
        decision: FabricDecision | None = None
        try:
            _done = [s for s in outcome.steps if s.get("ok")]
            _progress = "、".join(
                f"{i + 1}.{s.get('tool')}"
                for i, s in enumerate(outcome.steps))
            messages[0] = {"role": "system", "content": _system_origin + (
                f"\n\n【任务进度】已完成 {len(outcome.steps)} 步：{_progress or '（尚未执行）'}。"
                f"\n当前任务：{intent}。"
            ...
            decision = _local_decision_call(messages)
            if decision is None:
                # 本地不可用/失败 → 云端紧凑重试（旧逻辑兜底）
                retry_user = json.dumps(messages, ensure_ascii=False)
        ...
        if not operation:
            ...
            try:
                empty_retry = structured_output.invoke(
                    FabricDecision,
                    legacy=lambda u=json.dumps(messages, ensure_ascii=False): chat_fn(
