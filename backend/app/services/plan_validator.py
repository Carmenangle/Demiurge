...
# 「生图提示词/出图提示词」里的「生图/出图」是提示词的**定语**，不是生成动作——
# 2026-09-11 实锤：「顺便跟我讨论各个服装下的这名角色的生图提示词」被意图守卫的
# 「生图」命中 → 讨论型请求被断言必须有 submit/collect 步骤 → 计划编译死路。
# 判定前先剥这类复合词（单一属主在此，plan_compiler.is_delegation_intent 复用）。
_PROMPT_COMPOUND_RE = re.compile(
    r"(?:生图|出图|绘图|画图|生成图)(?:提示词|词|思路|参考|方向|要点|句式|模板)")


def strip_prompt_compounds(text: str) -> str:
    """剥掉「X图提示词」类复合词，留下纯文本供生成意图判定。"""
    return _PROMPT_COMPOUND_RE.sub("", text or "")


def _requires_generation(intent: str) -> bool:
    """意图文本是否要求出图/提交（复合词剥离后判定，防「生图提示词」误命中）。"""
    return bool(re.search(r"出图|生图|生成图|生成图片|生成视频|提交",
                          strip_prompt_compounds(intent)))
...
def validate(plan: GenerationPlan, *,
    ...
    if not plan.steps:
        ...
        if _requires_generation(plan.intent):
...
def _check_placeholders(label: str, params: dict) -> list[str]:
    ...
    **内容宏豁免（2026-09-10）**：`{{user}}`/`{{char}}`/`{{lastUserMessage}}`/`{{lastCharMessage}}`
    是 ST 惯例的**内容**宏（本项目按用户定案把主角名存成 `{{user}}` 写进世界书/角色卡），
    不是「未解析占位符」。此前一律判占位符 → 固化04 产物必然带 `{{user}}` → `solidify_steps`
    每次被拒，**配方从未产出过**（`fabric.solidified status=skipped`）。现先剥登记宏再看残留。
    名单单一属主 = `preset_store.CONTENT_MACRO_NAMES`，不得在此另开一份。
    """
    from app.services import preset_store  # 惰性导入：与 capability_registry 同风格

    errors: list[str] = []

    def _is_placeholder(value: str) -> bool:
        # 剥掉登记的内容宏后仍残留 {{ 与 }} → 真占位符（如 {{step1.output}}）
        residue = preset_store.strip_content_macros(value)
