...
def test_讨论生图提示词不是委派(monkeypatch):
    """2026-09-11 实锤：「跟我讨论各服装下的生图提示词」被「生图」二字拽进机械计划。

    「生图提示词」里的「生图」是提示词的定语，不是生成动作——讨论型请求
    不该进委派；而「批量出 N 张」这类真生成动作不受剥词影响。
    """
    msg = ("我要想根据这个角色设计各个情况下的服装,能否参考这个文档的服装设计内容,"
           "顺便跟我讨论各个服装下的这名角色的生图提示词,其中我会提供这各角色的参考图")
    assert not plan_compiler.is_delegation_intent(msg)
    # 真生成动作仍在：剥词不误伤
    assert plan_compiler.is_delegation_intent("参考文档的生图提示词，批量出 10 张图")
    assert plan_compiler.is_delegation_intent("按各套出图提示词分批生成图片")


def test_设计讨论型判定():
    """讨论动词+设计话题 → 智能编造自由循环（非机械计划、非剧情楼层）。"""
    msg = ("我要想根据这个角色设计各个情况下的服装,能否参考这个文档的服装设计内容,"
           "顺便跟我讨论各个服装下的这名角色的生图提示词")
    assert plan_compiler.is_design_discussion_intent(msg)
    assert plan_compiler.is_design_discussion_intent("跟我商讨一下这套服装的配色方案")
    # 讨论接续语（2026-09-11补）：「就按这套来」无典型讨论动词，漏判会掉回剧情楼层
    assert plan_compiler.is_design_discussion_intent("就按这套来，把场景1的提示词细化")
    assert plan_compiler.is_design_discussion_intent("按这套继续，扩写场景二的配色")
    # 非设计域的扩写不误伤（大纲/章节不在设计话题词表）
    assert not plan_compiler.is_design_discussion_intent("帮我把这份大纲扩写成三章并落盘")


def test_设计讨论粘性接续判定():
    """讨论接续轮不再靠枚举接续词（2026-09-11 实锤）。

    「不用ComfyUI的提示词,我需要的是适配gpt,banana这种ai模型的文字描述内容」
    无任何讨论动词/接续词 → 当晚三个判定全不命中 → supervisor 掉回 roleplay，
    灵感卡/规范/附件注入整段失效（隔一轮就失忆）。
    """
    recent = [
        "我想要根据这名角色设计各个情况下的服装,能否参考这个文档的服装设计内容,"
        "跟我商讨各个服装下的这名角色的生图提示词",
        "好，我已读取了文档与灵感参考",
    ]
    cur = "不用ComfyUI的提示词,我需要的是适配gpt,banana这种ai模型的文字描述内容"
    assert plan_compiler.is_design_discussion_continuation(cur, recent)
    assert not plan_compiler.is_design_discussion_intent(cur)   # 原判定确实兜不住
    # 无讨论窗口 → 不接续
    assert not plan_compiler.is_design_discussion_continuation(cur, ["帮我整理一下资料"])
    # 窗口在但本轮纯剧情叙述（无设计话题词）→ 不抢道
    assert not plan_compiler.is_design_discussion_continuation(
        "她提笔画了一幅像，转身离去", recent)
    # 窗口在但本轮是明确委派 → 委派优先
    assert not plan_compiler.is_design_discussion_continuation(
        "按这套提示词批量出 10 张图", recent)
    # 纯剧情/无讨论动词不命中（防绑卡作品的剧情对话被抢道）
    assert not plan_compiler.is_design_discussion_intent("她提笔画了一幅像，转身离去")
    assert not plan_compiler.is_design_discussion_intent("继续推进剧情")
    assert not plan_compiler.is_design_discussion_intent("")
