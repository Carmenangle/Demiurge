...
def test_build_png_card_roundtrip_无原图():
    """2026-09-06：JSON 卡 → PNG(ccv3 内嵌) → 读回一致（character.export_png 基础）。"""
    card = {"spec": "chara_card_v2", "data": {"name": "导出测试卡", "description": "roundtrip"}}
    png = cc.build_png_card(card)
    assert png.startswith(cc.PNG_SIGNATURE)
    parsed = cc.parse_card_bytes(png, "导出测试卡.png")
    assert parsed.name == "导出测试卡"
    assert parsed.description == "roundtrip"


def test_build_png_card_带原图剥离旧卡数据():
    """2026-09-06：带原图导出必须剥离原图旧 ccv3/chara，否则读端后覆盖前读到旧卡。"""
    old = {"spec": "chara_card_v2", "data": {"name": "旧卡"}}
    new = {"spec": "chara_card_v2", "data": {"name": "新卡"}}
    avatar = cc.build_png_card(old)  # 模拟原图已含卡数据
    png = cc.build_png_card(new, avatar)
    parsed = cc.parse_card_bytes(png, "新卡.png")
    assert parsed.name == "新卡", "带原图导出必须覆盖旧卡数据"


def test_export_png_card_handler(tmp_path):
    """2026-09-06：handler 落盘 <卡目录>/<卡名>.png 且可读回。"""
    from app.services import capability_handlers as ch

    card = {"spec": "chara_card_v2", "data": {"name": "作品卡", "first_mes": "开场白"}}
    ch.upsert_repo_character(base=str(tmp_path), card=card)
    res = ch.export_png_card(name="作品卡", base=str(tmp_path))
    assert res["name"] == "作品卡"
    assert "作品卡.png" in res["path"]
    parsed = cc.parse_card_bytes(open(res["path"], "rb").read(), "作品卡.png")
    assert parsed.name == "作品卡"
    assert (parsed.first_mes or "").startswith("开场白")
