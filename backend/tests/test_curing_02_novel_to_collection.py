...
def test_02_分卷读完整本小说_offset续读(tmp_path):
    """§2 分卷读（2026-09-06 实锤修复后）：长文本按 offset 逐卷读完，拼接=原文。"""
    _setup_work(tmp_path)  # 建立作品域
    # 生成长文（模拟中长篇小说）
    long = tmp_path / "long_novel.md"
    long.write_text("机制段落内容。" * 12000, encoding="utf-8")  # 固定 7 字符/段 × 12000 = 84000 字符
    total = len(long.read_text(encoding="utf-8"))
    assert total > 40000  # 必须长于两卷

    chunks = []
    offset = 0
    pages = 0
    while True:
        out = ch.read_text_file(str(long), max_chars=20000, offset=offset)
        chunks.append(out["text"])
        offset = out["offset"] + out["chars_read"]
        pages += 1
        assert pages <= 10  # 防死循环
        if not out["has_more"]:
            break
    assert "".join(chunks) == long.read_text(encoding="utf-8")
    assert pages == 5  # 84000 字符 / 20000 = 4.2 → 5 页
