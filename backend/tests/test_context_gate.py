...
# ── keep 档 token 预算（P2b，2026-09-12）：同域长讨论不再无限膨胀 ──

def test_keep档超预算旧段压成任务进度摘要():
    """近期一半预算原文保留，更早 keep 段 LLM 压成进度摘要（auto-compact 同机制）。"""
    keeps = [(("用户" if i % 2 == 0 else "助手"), f"讨论要点{i}" + "细" * 600)
             for i in range(8)]
    seen = {}

    def fake_summarizer(items):
        seen["count"] = len(items)
        return "已确认：场景1用银粉渐变发色；待办：场景2配色未定"

    lines = gate._keep_budget_lines(keeps, 1_500, fake_summarizer)

    assert seen["count"] >= 1
    assert any("任务进度摘要" in line and "银粉渐变发色" in line for line in lines)
    assert lines[-1].endswith("讨论要点7" + "细" * 600)      # 最新原文保留在尾
    body = "\n".join(lines)
    from app.services import agent_context as _ac
    assert _ac.estimate_tokens(body) <= 1_500 + 80            # 预算内（留标注余量）


def test_keep档摘要失败退机械聚合():
    keeps = [(("用户" if i % 2 == 0 else "助手"), f"讨论要点{i}" + "细" * 600)
             for i in range(8)]

    lines = gate._keep_budget_lines(keeps, 1_500, lambda items: None)

    assert any("相关·聚合" in line and "此前同域讨论" in line for line in lines)
    assert lines[-1].endswith("讨论要点7" + "细" * 600)


def test_keep档未超预算原样保留():
    keeps = [("用户", "短消息一"), ("助手", "短消息二")]

    lines = gate._keep_budget_lines(keeps, 4_000, lambda items: "不该被调用")

    assert lines == [f"[相关·保留] {role}：{text}" for role, text in keeps]


def test_keep档预算关闭时行为不变():
    keeps = [(("用户" if i % 2 == 0 else "助手"), f"讨论要点{i}") for i in range(10)]

    lines = gate._keep_budget_lines(keeps, 0, lambda items: "不该被调用")

    assert len(lines) == 10
    assert all(line.startswith("[相关·保留]") for line in lines)
