...
def test_full模式doc_create_repo端到端(store, monkeypatch):
    monkeypatch.setattr(plan_tasks, "_agent_access_mode",
                        lambda: capability_sandbox.ACCESS_FULL)
    # 文档落**作品域**（仓库/小仓库文件夹）：`<作品库根>/<作品文件夹名>/docs/`，
    # 不再是作品库根的全局单例 `docs/`（2026-09-10 B3/A）。用已知仓库名钉死落点。
    from app.services import repo_meta
    monkeypatch.setattr(repo_meta, "_load_state",
                        lambda: {"repos": [{"id": "work", "name": "里程碑作品"}]})
    out_dir = str(store["path"].parent)
    # 2026-09-10（A2）：rel_path 只收文件名——文档一律平铺 docs/ 下（带目录被拒）
    plan = _plan([PlanStep(id="s1", operation="doc.create_repo",
                           params={"rel_path": "里程碑.md",
                                   "content": "# 里程碑\n\n已落地两档访问标准"})])
    ...
    written = Path(out_dir).joinpath("里程碑作品", "docs", "里程碑.md")
    assert written.read_text(encoding="utf-8") == "# 里程碑\n\n已落地两档访问标准"
    assert not Path(out_dir).joinpath("docs").exists()
...
def test_审批与重排队列均发布进度(store, fake_capabilities):
    """2026-09-09 江瑶 14 套实锤修复：计划停 awaiting_approval / 被 requeue 后，
    后台活动面板进度必须跟着状态走（此前停在 2/4 步·图 0/14 不刷新）。"""
    plan = _plan([PlanStep(id="s1", operation="test.batch", params={"template_id": "t1"})])
    task_id = plan_tasks.submit_task(plan, output_dir=str(store["path"].parent),
                                     configured_models={"chat", "image"})["task_id"]
    # 无租约首跑 → 停在待批，进度发布 awaiting_approval
    plan_tasks._run_task(plan_tasks._claim_next(), threading.Event())
    assert plan_tasks.get_task(task_id)["status"] == "awaiting_approval"
    assert store["progress"][task_id]["status"] == "awaiting_approval"
    # 批准 → 状态转 queued，进度同步发布 queued（不再卡旧值）
    plan_tasks.approve_task(task_id)
    assert store["progress"][task_id]["status"] == "queued"
    # 批准后续跑 → 终态 done 且进度发布 done
    plan_tasks._run_task(plan_tasks._claim_next(), threading.Event())
    task = _wait_done(store, task_id)
    assert task["status"] == "done"
    assert store["progress"][task_id]["status"] == "done"


def test_requeue发布进度并清残留错误(store, fake_capabilities):
    """2026-09-09 修复：_requeue 把任务转回 queued 时发布进度并清空 error，
    避免前端显示「失败原因 + 实际已排队」的矛盾状态。"""
    plan = _plan([PlanStep(id="s1", operation="test.echo", params={"a": "1"})])
    task_id = plan_tasks.submit_task(plan, output_dir=str(store["path"].parent),
                                     configured_models={"chat", "image"})["task_id"]
    with plan_tasks.get_connection() as connection:
        connection.execute("update plan_tasks set status='partial', error='[boom] 旧错误' "
                           "where id=?", (task_id,))
        connection.execute("update plan_task_steps set status='failed' where task_id=?",
                           (task_id,))
    store["progress"].pop(task_id, None)
    assert plan_tasks.retry_step(task_id, "s1") is True  # 内部走 _set_step_status + _requeue
    task = plan_tasks.get_task(task_id)
    assert task["status"] == "queued" and task["error"] == ""
    assert store["progress"][task_id]["status"] == "queued"
    # 重跑成功 → 终态 done，进度终值正确
    plan_tasks._run_task(plan_tasks._claim_next(), threading.Event())
    assert _wait_done(store, task_id)["status"] == "done"
...
def test_认领清除残留error(store, fake_capabilities):
    """2026-09-09 修复：_claim_next 认领时清空历史 error（含上一次 needs_approval
    残留），worker 重新执行时 DB/活动面板不再显示旧错误。"""
    plan = _plan([PlanStep(id="s1", operation="test.echo", params={"a": "1"})])
    task_id = plan_tasks.submit_task(plan, output_dir=str(store["path"].parent),
                                     configured_models={"chat", "image"})["task_id"]
    with plan_tasks.get_connection() as connection:
        connection.execute("update plan_tasks set error='[needs_approval] 旧租约错误' "
                           "where id=?", (task_id,))
    claimed = plan_tasks._claim_next()
    assert claimed is not None
    with plan_tasks.get_connection() as connection:
        row = connection.execute("select error, status from plan_tasks where id=?",
                                 (task_id,)).fetchone()
    assert row["status"] == "running" and row["error"] == ""
...
def test_含内容宏的轨迹可固化(store):
    """回归（2026-09-10）：产物含 `{{user}}` 时固化必须成功、配方必须真的产出。

    固化04 产物**必然**带 `{{user}}`——`capability_handlers` 是按用户定案主动把主角名
    替换成 `{{user}}` 写进世界书的，合并后的文档自然带这个宏。此前
    `plan_validator._check_placeholders` 把「value 里同时出现 `{{` 与 `}}`」一律判为
    未解析占位符 → `solidify_steps` 每次 `ValueError`，被 `agent_graph._fabric_finalize`
    吞成 `fabric.solidified status=skipped`：**配方从未产出过**（`plan_recipes.json`
    至今不存在），前端「保留/不保留」内联卡也不出现。内容宏豁免后应正常出草稿。
    """
    root = str(store["path"].parent)
    recipe = plan_tasks.solidify_steps(
        intent="把世界书条目/角色卡外貌/近期纪要合并成设定总集", output_dir=root,
        name="设定总集合并",
        steps=[{"tool": "doc.create_repo",
                "params": {"rel_path": "设定总集.md",
                           "content": "# 设定总集\n\n{{user}} 扮演的主角，{{char}} 的搭档。"},
                "ok": True}])
    assert recipe["status"] == "draft" and recipe["origin"] == "fabric"
    assert recipe["plan"]["steps"][0]["params"]["content"].count("{{user}}") == 1


def test_含真引用的轨迹仍不固化(store):
    """豁免只针对登记宏：`{{step1.output}}` 这类引用前序步骤的占位符仍必须被拒。"""
    root = str(store["path"].parent)
    with pytest.raises(ValueError, match="占位符"):
        plan_tasks.solidify_steps(
            intent="引用前序产出", output_dir=root,
            steps=[{"tool": "doc.create_repo",
                    "params": {"rel_path": "a.md", "content": "{{step1.output}}"},
                    "ok": True}])
...
def test_migrate_mechanical的base与repo_id由环境归一(store):
    """2026-09-09 根因修复验收：ST 机械转写纳入计划编排后，模型只给 path，
    base/repo_id 一律由 submit_task 按运行环境覆盖（防写入 <作品名>/worldbook.json 分叉）。"""
    root = str(store["path"].parent)
    src = store["path"].parent / "src.png"
    src.write_bytes(b"stub")
    plan = _plan([PlanStep(id="s1", operation="character.migrate_mechanical",
                           params={"path": str(src), "base": "D:/evil",
                                   "repo_id": "作品名"})], intent="转成合集卡")
    out = plan_tasks.submit_task(plan, output_dir=root, repo_id="repo-9")
    task = plan_tasks.get_task(out["task_id"])
    params = task["steps"][0]["params"]
    assert params["base"] == root and params["repo_id"] == "repo-9"
    assert params["path"] == str(src)


def test_终态done清空历史错误(store):
    """2026-09-09 实锤：重试成功后 status=done 但 error 仍留着旧的 needs_approval，
    前端会显示「完成却报错」。终态成功/取消必须清空 error（partial 保留诊断）。"""
    root = str(store["path"].parent)
    plan = _plan([PlanStep(id="s1", operation="file.list_dir", params={"path": root})],
                 intent="列目录")
    out = plan_tasks.submit_task(plan, output_dir=root)
    task_id = out["task_id"]
    with plan_tasks.get_connection() as connection:
        connection.execute("update plan_tasks set error=? where id=?",
                           ("[needs_approval] 旧错误", task_id))
        connection.execute("update plan_task_steps set status='done' where task_id=?",
                           (task_id,))
    plan_tasks._finalize(task_id, [], 1)
    task = plan_tasks.get_task(task_id)
    assert task["status"] == "done" and task["error"] == ""


def test_embed_worldbook与export_to_library的base_repo_id由环境归一(store):
    """2026-09-09 实锤：这两个能力漏加环境归一 → 执行时 handler 缺必需参数 TypeError，
    任务停在 partial、用户看不到产物。提交期归一后，计划卡与执行看到的都是真实值。"""
    root = str(store["path"].parent)
    plan = _plan([
        PlanStep(id="s1", operation="character.embed_worldbook",
                 params={"base": "D:/evil", "repo_id": "作品名"}),
        PlanStep(id="s2", operation="workspace.export_to_library",
                 params={"base": "D:/evil", "repo_id": "作品名"}),
    ], intent="内嵌并同步源库")
    out = plan_tasks.submit_task(plan, output_dir=root, repo_id="repo-9")
    task = plan_tasks.get_task(out["task_id"])
    for step in task["steps"]:
        assert step["params"]["base"] == root
        assert step["params"]["repo_id"] == "repo-9"
