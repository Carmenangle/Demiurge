...
def test_纪要历史缺口窗口对齐不合并跨频率大卡(tmp_path):
    """2026-09-13 用户裁决：落后超频不再死锁，窗口对齐只抽最近 cadence 个回合，
    不回溯缺口（防合并跨频率大卡、防引用已删楼层）。"""
    deps = _deps(
        tmp_path,
        lambda *a, **k: '{"overview":"最近窗口","chronicle":"最近三回合事件","keywords":[]}')
    ...
    assert (entries[0].turn_start, entries[0].turn_end) == (12, 14)  # 只盖最近窗口，缺口留空
    assert ns.get_last_turn(str(tmp_path), "r1", "卡A") == 14


def test_首条抽取不受落后死锁_last0窗口对齐(tmp_path):
    """last=0 且 turn 远超 cadence（原 manual_backfill_required 永久卡死场景）：
    直接抽最近 cadence 窗口一条，进度推进到当前回合。"""
    deps = _deps(
        tmp_path,
        lambda *a, **k: '{"overview":"开局概览","chronicle":"最近三回合事件","keywords":[]}')
    got = ra.maybe_summarize(
        deps, repo_id="r1", card_name="卡A", window_text="最近几轮",
        turn=36, chat_base="b", chat_key="k", chat_model="m", cadence=3,
    )
    assert got is True
    entries = ns.all_entries(str(tmp_path), "r1")
    assert len(entries) == 1
    assert (entries[0].turn_start, entries[0].turn_end) == (34, 36)
    assert ns.get_last_turn(str(tmp_path), "r1", "卡A") == 36
