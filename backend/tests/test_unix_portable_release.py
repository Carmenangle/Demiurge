...
def _assets(tmp_path: Path, target: str, edition: str) -> Path:
    ...
    (assets / f"Demiurge-update-v0.15-{target}.json").write_text(
...
def test_macos_bundle_contains_runtime_rag_and_clickable_start_file(tmp_path):
    ...
    assert outputs[0].name == "Demiurge-00-USER-DOWNLOAD-macOS-ARM64-Full-RAG-v0.15.tar.gz"
    with tarfile.open(outputs[0], "r:gz") as bundle:
        names = set(bundle.getnames())
        start = next(member for member in bundle.getmembers() if member.name.endswith("Start-Demiurge.command"))
    assert any(name.endswith("data/runtime/rag/rag-a/site-packages/torch/__init__.py") for name in names)
    assert start.mode & 0o111


def test_linux_full_rag_tree_contains_shell_start_file(tmp_path):
    assets = _assets(tmp_path, "linux-x64-full-rag", "full-rag")
    tree, _, manifest = unix_release.build_tree(
        assets, "v0.15", tmp_path / "work", "linux-x64-full-rag",
    )

    assert (tree / "start-demiurge.sh").is_file()
    assert manifest["edition"] == "full-rag"
