...
def test_execute_headless保留直连剧情上下文参数(tmp_path, monkeypatch):
    ...
    queue._execute({
        ...
        "context_count": 9, "selfheal_attempts": 2,
    ...
    assert context.history_override == history and context.context_count == 9
    ...
    assert context.search_provider == "bing-cn"  # 联网搜索源随即时/队列两条路一起透传
