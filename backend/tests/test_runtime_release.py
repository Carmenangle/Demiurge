...
def test_frontend_closure_copies_shared_and_comfyui_ext(tmp_path):
    root = tmp_path / "root"
    (root / "frontend" / "src" / "lib").mkdir(parents=True)
    (root / "frontend" / "src" / "types").mkdir(parents=True)
    (root / "shared" / "fixtures").mkdir(parents=True)
    (root / "comfyui-ext" / "laf_lock" / "js").mkdir(parents=True)
    (root / "frontend" / "src" / "lib" / "a.test.ts").write_text(
        'import p from "../../../shared/fixtures/f.json";\n'
        'import s from "../../../comfyui-ext/laf_lock/js/l.js?raw";\n'
        'import t from "../types/chat";\n',
        encoding="utf-8",
    )
    (root / "frontend" / "src" / "types" / "chat.ts").write_text(
        "export {};", encoding="utf-8"
    )
    (root / "shared" / "fixtures" / "f.json").write_text("{}", encoding="utf-8")
    (root / "comfyui-ext" / "laf_lock" / "js" / "l.js").write_text(
        "export {};", encoding="utf-8"
    )
    ...
    with pytest.raises(RuntimeError, match="越根引用"):
        runtime_release._assert_frontend_closure(frontend_work)

    runtime_release._copy_frontend_cross_root_sources(root, work_dir)
    assert (work_dir / "shared" / "fixtures" / "f.json").is_file()
    assert (work_dir / "comfyui-ext" / "laf_lock" / "js" / "l.js").is_file()
    runtime_release._assert_frontend_closure(frontend_work)
