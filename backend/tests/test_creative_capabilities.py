...
def test_upsert_repo_handler把key归一并丢弃杂字段(tmp_path):
    ...
    out = ch.upsert_repo_worldbook(str(tmp_path), "work", [
        {"key": "加恩", "content": "正文", "junk": 1, "comment": "角色卡·加恩"},
...
def test_upsert_repo_handler缺comment跳过不落无名条目(tmp_path):
    """2026-09-09 用户实锤（界面出现「条目 25/39/40」）：comment 是条目名与去重锚点，
    缺它一律跳过并回填补名提示——不再写入无名条目（旧行为是 warning 后照写）。"""
    out = ch.upsert_repo_worldbook(str(tmp_path), "work", [
    ...
    from app.services import worldbook_store
    # 无写入 → 快照不落盘（避免空书覆盖）
    assert worldbook_store.read_repo_snapshot(str(tmp_path), "work") is None
...
def test_doc_create写作品docs并拒绝穿越(tmp_path):
    out = ch.create_repo_doc(str(tmp_path), "start.md", "# 开始\n\n你好")
    assert tmp_path.joinpath("docs", "start.md").read_text(encoding="utf-8") == "# 开始\n\n你好"
    assert out["bytes"] > 0
    ...
    with pytest.raises(FileExistsError):
        ch.create_repo_doc(str(tmp_path), "start.md", "再来")


def test_doc_create拒绝带目录的rel_path(tmp_path):
    """2026-09-10 链路③：产物白名单只认 `docs/*.md`（恰一层），而 create_repo 原先允许
    任意深度——`sub/x.md` 能写成功却收集不到、预览 403，是「假成功」。收窄为只接受文件名。"""
    with pytest.raises(ValueError, match="带目录|文件名"):
        ch.create_repo_doc(str(tmp_path), "guide/start.md", "x")
    with pytest.raises(ValueError, match="带目录|文件名"):
        ch.create_repo_doc(str(tmp_path), "docs/start.md", "x")
    # 收窄后只可能落在 docs/ 顶层，白名单必放行（能力面 ⊆ 白名单面）
    from pathlib import Path

    from app.services.collection_artifacts import is_allowed_artifact
    out = ch.create_repo_doc(str(tmp_path), "总集.md", "# x")
    assert is_allowed_artifact(Path(out["path"]), tmp_path)
