...
def test_frontmatter_技能头解析_读与列举(know_dir):
    _write(know_dir, "固化02-规范.md",
           ...
           "tools: [novel.survey, novel.charfacts]\n"
           "---\n# 正文标题\n内容")
...
def test_真实固化技能文档_本地DATA_DIR存在则契约校验():
    """真实固化技能文档（backend/data/agent_knowledge/，gitignored 运行态）——
    目录不存在（CI/干净环境）时跳过，存在时逐一校验每份技能文档完整可装载。
    份数断言用「≥3」并随库增长自动放宽（固化04 起库会继续长），不做名单硬编码；
    _assert_doc_tools_covered 的 tools 漂移防护必须保留。"""
    ...
    assert len(curing) >= 3, f"固化技能文档至少 3 份，实际 {len(curing)}：" \
        f"{sorted(curing)}"
    catalog = agent_graph._knowledge_catalog_text()
