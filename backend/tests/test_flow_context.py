...
def test_handler_registers_only_with_repo_id(tmp_path, monkeypatch):
    """文档落**作品域**（仓库文件夹），不是作品库根的全局单例 docs/（2026-09-10 B3/A）。"""
    from app.services import repo_meta
    monkeypatch.setattr(repo_meta, "_load_state",
                        lambda: {"repos": [{"id": "t9", "name": "出图作品"}]})
    base = tmp_path / "work"
    base.mkdir()
    capability_handlers.create_repo_doc(
        str(base), "出图清单-角色A.md", "# 出图清单\n1 张", repo_id="t9")
    target = base / "出图作品" / "docs" / "出图清单-角色A.md"
    assert target.is_file()
    assert not (base / "docs").exists()      # 不再写全局单例
    ...
    # 不带 repo_id（旧式直调）→ 落作品库根 docs/、不登记、不抛错
    capability_handlers.create_repo_doc(str(base), "b.md", "# B")
    assert (base / "docs" / "b.md").is_file()
