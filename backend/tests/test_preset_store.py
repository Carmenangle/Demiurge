...
def test_setvar定义剥离且跨片段注册():
    preset = _preset(
        [("defs", True), ("text", True)],
        [
            {"identifier": "defs", "marker": False,
             "content": "{{setvar::fmt::<think>X</think>}}"},
            {"identifier": "text", "marker": False,
             "content": "正文引用{{fmt}}结束"},
        ],
    )
    out = preset_store.assemble_system(preset, {})
    assert "{{setvar::" not in out
    assert out == "正文引用<think>X</think>结束"


def test_setvar嵌套值配平与后定义覆盖():
    scope: dict[str, str] = {}
    out = preset_store.expand_setvar(
        "{{setvar::a::V1}}{{setvar::b::X{{a}}Y}}{{setvar::a::V2}}前{{a}}中{{b}}", scope,
    )
    assert scope == {"a": "V2", "b": "XV1Y"}  # 值内引用取定义当时的 a=V1；后定义覆盖
    assert out == "前V2中XV1Y"


def test_setvar_getvar与未注册保留():
    scope: dict[str, str] = {}
    out = preset_store.expand_setvar(
        "{{setvar::重置::[RESET]}}{{getvar::重置}}|{{getvar::无}}|{{未注册}}|{{user}}", scope,
    )
    assert out == "[RESET]||{{未注册}}|{{user}}"
    # user/char 等宏不在 setvar 层展开（由 substitute_macros 处理）；未注册引用保留原样


def test_assemble_messages_setvar不泄漏且user片段保留role():
    preset = _preset(
        [("defs", True), ("prompt", True)],
        [
            {"identifier": "defs", "marker": False,
             "content": "{{setvar::链::格式}}"},
            {"identifier": "prompt", "marker": False, "role": "user",
             "content": "思考{{链}}后输出"},
        ],
    )
    msgs = preset_store.assemble_messages(preset, {})
    assert [m["role"] for m in msgs] == ["user"]
    assert msgs[0]["content"] == "思考格式后输出"
    assert "{{setvar::" not in msgs[0]["content"]
