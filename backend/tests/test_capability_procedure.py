...
def test_approval租约空capabilities起步零授权而full放行():
    # 2026-09-06：approval 自由循环允许空 capabilities 起步（模型逐步调用时由
    # grant_operation 在用户批准后逐条追加），未授权操作 authorize 一律拒绝。
    capability_sandbox._reset_for_tests()
    empty_lease = capability_sandbox.grant("plan:empty", [])
    with pytest.raises(PermissionError):
        capability_sandbox.authorize(empty_lease["id"], "file.write")
    capability_sandbox.grant_operation(empty_lease["id"], "file.write", path="")
    capability_sandbox.authorize(empty_lease["id"], "file.write")
    lease = capability_sandbox.grant(
