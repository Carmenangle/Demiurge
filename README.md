...
在 [GitHub Releases](https://github.com/Carmenangle/Demiurge/releases) 下载与你平台匹配的包
（`Demiurge_windows_*` / `Demiurge_macos_*` / `Demiurge_linux_*`；已内置 Python 运行时、
...
- 支持导入与转换 **SillyTavern** 角色卡、世界书、预设与正则，并可直接编辑保存。
- 对话与画布是同一作品的两个视图，在功能栏点切换按钮（⇄ 图标）即可切换。
  （旧版「顶部工作模式三选一：剧情模式 / 多元数据生成 / 编辑模式」**已废弃**，入口已合并进顶栏功能栏。）
