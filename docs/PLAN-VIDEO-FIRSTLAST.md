...
1. **可实测的视频端点**：现 `https://ai.t8star.org/v2/videos/generations` 是否可用于双图首尾帧实测？
   （决定 R1 的最终字段名与 R9 的 size 白名单）
2. **转场素材是否入一期**：`preset.transitionImage`（接不上时首帧兜底）——入 P4 还是延后？
3. **元信息是否写模型名**：R3 中「Minimax H3」是否硬编码进提示词，还是从 `video_config.model` 透传？
...
```
P1 锚点提取 ──┐
              ├─→ P5 前端提交链 ──→ P6 真实 API 对齐
P2 表格读取 ──┤
P3 videoMode/协议/守卫(含 R2 R3 R6 R7) ──┘
P4 尾帧反查（可并行，独立）
```

先 P3（含 R2/R3/R6/R7 的守卫与修正）——因为它同时是 P1/P2 的「出口契约」，
定了 `videoMode` 与事件字段后，P1/P2 的产出才有明确的落点。
