...
进入作品后点功能栏的对话/画布切换按钮（⇄ 图标）即可在对话与画布视图之间切换。

> **已作废信息（勿再引用）**：本文此前写的「顶部工作模式三选一：剧情模式 / 多元数据生成 / 编辑模式」**已废弃**——
> 该顶部切换器早已合并进顶栏功能栏（`frontend/src/App.tsx` 品牌区注释「模式下拉已合并」）。
> 旧链接 `#/story`、`#/generate`、`#/code` 仍作兼容解析，但不再是用户可选入口。

![进入画布](../assets/guide/canvas-1.png)
