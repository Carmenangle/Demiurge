...
- 触发条件：某真实 Provider 文档+实测确认参考图字段后，`generate()` 加可选 `image` 参数，payload 条件注入。
...
> **进度（2026-08-20）**：基础下载链路已落地（`image_store.save_web_material` 下载到 `_web_materials/`，`/comfyui/web-materials/save|list|delete` 路由，`WebMaterialsView` 前端网格视图+批量删除/发送至画布）；但**安全校验链尚未完成**——缺魔数校验、SSRF/私网 IP 拒绝、域名策略、大小上限、provenance 完整记录、`.part` 临时文件原子落盘、后台任务化、候选列表校验。当前实现是初步下载存储，不是合同定义的「受控下载」。
