...
- **climax**：高潮桥段提取 → 1 张高潮画面 → 单图生视频（现状）。
- **firstlast**：剧情楼层（roleplay 路由产出，参考 `isStoryNode` 同思路）→ 生首帧图 + 尾帧图 → 双图生视频。
...
**核心工作在第⑤块**：把剧情动作窗口按时间轴切 3-5 段，每段「节奏名 + 运镜 + 主体动作 + 特效/节拍」，
这是图片提示词没有的。产出 `video_prompt` 随 `illustrate_request` 下发，前端视频分支优先用。

**首尾帧模式提示词**：在七段式基础上，③ 参考绑定扩展为首帧+尾帧两张图，⑤ 时间分镜覆盖
「首帧画面 → 演变 → 尾帧画面」全程；首帧描述带「上楼层尾帧」上下文（V1.5 决策②）。
