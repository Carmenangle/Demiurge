...
**目标**：视频参考图从「首帧=角色底图」扩展到「首尾帧」——首帧定格角色、尾帧定格目标画面。

**现状**：首帧已由 V1.1 的「角色底图注入」达成（每角色参考图 = 该角色底图 `characterLoras[角色].baseImage`）。**尾帧作为后续目标**，一期先使用角色底图生成的形式。

**预排问题**：
- **尾帧语义**：尾帧是「目标画面」还是「下一镜头衔接」需先定；尾帧来源（插画/用户指定/剧情目标帧）尚未定义，不猜。
- **依赖**：尾帧需要「目标帧提取」能力，与视频独特提示词方法（V1.6）强耦合，一并规划。

### V1.6 视频独特提示词方法（后续探讨）

**目标**：视频提示词从「高潮动作概括」升级为「视频专属生成法」。

**现状**：视频提示词在高潮动作时段提取，但不能简单用高潮动作概括——视频有独特的分镜/运镜/时序生成方法。

**预排问题**：
- **方法未定**：视频提示词的镜头语言、时间轴、动作连续性如何建模，需单独探讨；不套用生图的画面静态描述框架。
- **与 V1.5 联动**：首尾帧 + 视频提示词共同决定一段视频的「起-承-合」，两者一起设计。
