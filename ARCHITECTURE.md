...
2. **会话生成生命周期（已完成）**：`generationLifecycle` 以正交的 Agent/ComfyUI 槽保存前台状态：Agent 完成后对话立即可继续，不能因仍在运行的工作流而排队；任一槽完成不得清掉另一槽。`workflowGenerationRuntime` 统一拥有多个 ComfyUI pending 的持久化、提交时仓库归属快照、轮询节奏、连续 `not_found`、恢复检查、超时保留和并发 finalize 双闸。后台活动面板只读 pending，不得删除生成真源；原图或会话槽未持久化时必须保留任务重试。
...
| 模块 | 职责 |
|------|------|
| `useChatSession` | 聊天会话引擎：messages、聊天/队列编排、快照与媒体落盘 Adapter。新输入只在 Agent 槽占用时进入持久队列；ComfyUI 槽占用时直接开始下一轮对话。删除、重新生成、显式历史、导航后台化和自动插画提交合同保持不变；ComfyUI pending/轮询/恢复/finalize 顺序委托 `workflowGenerationRuntime` | 不得重新内联 pending localStorage、轮询计数或恢复分支；不得用 `wfRunning` 阻塞普通对话；界面提示、媒体原位回填和进度 WS 留在 Hook |
